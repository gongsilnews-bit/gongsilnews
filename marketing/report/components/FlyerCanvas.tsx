    ...
    <div className={`group relative w-full h-full bg-[#0f172a]/95 rounded-2xl overflow-hidden shadow-md border border-gray-100 ${className}`}>
      ...
      {isUploading && (
        <div className="absolute inset-0 bg-slate-900/80 z-[25] flex flex-col items-center justify-center print:hidden">
          ...
          <span className="text-[10px] text-amber-500 font-bold">WebP 理쒖쟻솕 뾽濡쒕뱶 以...</span>
        </div>
      )}
    </div>
  );
...
const FlyerCanvas = forwardRef<HTMLDivElement, FlyerCanvasProps>(({ data, activeTab = 'all', onUpdateInfo, onImageUpload, isUploadingImage }, ref) => {
  ...
  const targetTitle = info.address || '꽌珥덈룞 뿭꽭沅 留ㅻℓ 븞궡꽌';
  const price = info.priceMain || '75뼲 썝';
  ...
  const addFloorStatusRow = () => {
    if (onUpdateInfo && Array.isArray(info.floorStatus)) {
      onUpdateInfo({
        ...
        floorStatus: [
          ...info.floorStatus,
          { floor: '깉 痢', purpose: '슜룄', lease: '엫李', status: '젏쑀긽깭', note: '鍮꾧퀬' }
        ]
      });
    }
  };
    ...
    <div className="flex flex-col items-center p-8 bg-gray-100" ref={ref}>
        ...
        >
            <div className="flex gap-8 h-full">
                ...
                <div className="w-5/12 flex flex-col justify-between">
                    <div>
                        <SectionTitle title="PROPERTY OVERVIEW" subtitle="臾쇨굔媛쒖슂" />
                        <div className="border-t-[3px] border-gray-800 flex flex-col text-sm border-b border-gray-200">
                            {(() => {
                                const rows = Array.isArray(info.overviewTable) 
                                    ...
                                    : [
                                        { k: '냼옱吏', v: info.overviewTable?.location },
                                        { k: '슜룄吏뿭', v: info.overviewTable?.zoning },
                                        { k: '吏硫댁쟻', v: info.overviewTable?.landArea },
                                        { k: '뿰硫댁쟻', v: info.overviewTable?.totalArea },
                                        { k: '嫄대Ъ洹쒕え', v: info.overviewTable?.buildingScale },
                                        { k: '二쇱슜룄', v: info.overviewTable?.mainPurpose },
                                        { k: '二쇱감닔', v: info.overviewTable?.parking },
                                        { k: '듅媛뺢린', v: info.overviewTable?.elevator },
                                        { k: '以怨듭뿰룄', v: info.overviewTable?.completionYear },
                                    ];
                                ...
                                return (
                                    <div className="flex bg-[#fff9f0] border-t border-gray-200 group relative">
                                        ...
                                        <div className="absolute -left-20 top-1/2 -translate-y-1/2 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200 print:hidden bg-white/95 shadow-md border border-gray-200 rounded-md p-1.5 z-30">
                                            <span className="text-[10px] text-gray-400 font-bold text-center border-b pb-0.5 mb-0.5">嫄곕옒 삎깭</span>
                                            {["留ㅻℓ", "쟾꽭", "썡꽭"].map((type) => (
                                                <button
                                                    key={type}
                                        ...
                                        <div className="w-2/3 text-[#cc5a27] font-extrabold py-3 pl-4 flex items-center">
                                            {tType === "썡꽭" || tType === "엫" ? (
                                                <div className="flex items-center gap-1">
                                                    ...
                                                    <span>/</span>
                                                    <EditableText value={info.priceSub || ''} onChange={(val) => handleTextChange('priceSub', val)} placeholder="썡꽭" />
                                                </div>
                                            ) : (
                                                <EditableText value={price} onChange={(val) => handleTextChange('priceMain', val)} />
                                            )}
                ...
                <div className="w-7/12 flex flex-col justify-between h-full">
                    ...
                    <div>
                        <SectionTitle title="INVESTMENT SUMMARY" subtitle="닾옄슂빟" />
        ...
        >
            <div className="flex gap-8 h-full">
                ...
                <div className="w-5/12 h-full flex flex-col">
                    <div className="text-gray-600 font-bold text-sm mb-4">1. 痢듬퀎 젏쑀 諛 엫 긽꽭 쁽솴</div>
                        ...
                        <div className="flex-1 overflow-auto">
                            <table className="w-full text-center text-sm">
                                <thead className="bg-white border-b border-gray-200 sticky top-0 z-10">
                                    <tr>
                                        <th className="py-4 font-bold text-gray-600">痢듭닔</th>
                                        <th className="py-4 font-bold text-gray-600">쁽슜룄</th>
                                        <th className="py-4 font-bold text-gray-600">엫李</th>
                                        <th className="py-4 font-bold text-gray-600">젏쑀 긽깭</th>
                                        <th className="py-4 font-bold text-gray-600">鍮꾧퀬</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-gray-100 bg-white">
                                    {info.floorStatus?.map((row, i) => (
                                        <tr key={i} className="group relative">
                                            <td className={`py-4 relative ${row.floor === 'B1' || row.floor.includes('吏븯') ? 'font-bold' : ''}`}>
                                            ...
                                            <td className={row.floor === 'B1' || row.floor.includes('吏븯') ? 'font-bold' : ''}>
                                                <EditableText value={row.purpose} onChange={(val) => updateFloorStatusRow(i, 'purpose', val)} />
                                            </td>
                                            {i === 0 && info.floorStatus[0].lease === '蹂댁쬆湲 / 李⑥엫 궡뿭 蹂꾨룄臾몄쓽' ? (
                                                 <>
                                                 ...
                                                 info.floorStatus[0].lease === '蹂댁쬆湲 / 李⑥엫 궡뿭 蹂꾨룄臾몄쓽' ? (
                                                     <>
                                                         <td className={row.status.includes('쁽옱 怨듭떎') ? 'text-[#cc5a27] font-bold' : 'font-bold'}>
                                                             <EditableText value={row.status} onChange={(val) => updateFloorStatusRow(i, 'status', val)} />
                                                         </td>
                                                         <td className={row.note.includes('利됱떆 솢슜') ? 'font-bold text-gray-800' : 'text-gray-500'}>
                                                             <EditableText value={row.note} onChange={(val) => updateFloorStatusRow(i, 'note', val)} />
                                                         </td>
                                                     </>
                                                  ) : (
                                                     <>
                                                         <td className={row.lease.includes('怨듭떎') ? 'text-[#cc5a27] font-bold' : 'font-bold'}>
                                                             <EditableText value={row.lease} onChange={(val) => updateFloorStatusRow(i, 'lease', val)} />
                                                         </td>
                                                         <td className={row.status.includes('怨듭떎') ? 'text-[#cc5a27] font-bold' : 'font-bold'}>
                                                             <EditableText value={row.status} onChange={(val) => updateFloorStatusRow(i, 'status', val)} />
                                                         </td>
                                                         <td className={row.note.includes('利됱떆') ? 'font-bold text-gray-800' : 'text-gray-500'}>
                                                             <EditableText value={row.note} onChange={(val) => updateFloorStatusRow(i, 'note', val)} />
                                                         </td>
                                                     </>
                                                  )
                ...
                <div className="w-7/12 h-full flex flex-col">
                    <div className="text-gray-600 font-bold text-sm mb-4">2. 留ㅺ컖 빑떖 븯씠씪씠듃 & 떆꽭 遺꾩꽍</div>
                    <div className="flex-1 border border-yellow-200 rounded-lg p-6 bg-white shadow-sm flex flex-col">
                        <h3 className="text-xl font-extrabold text-gray-900 mb-4 border-b-2 border-gray-800 pb-2 inline-block">留ㅺ컖 빑떖 븯씠씪씠듃</h3>
                        ...
                        <div className="mt-auto border-t-2 border-[#cc5a27] pt-6 relative">
                            ...
                            <div className="flex justify-around px-8 text-xs font-bold text-gray-500">
                                ...
                                <span>湲곗〈 씗留앷</span>
                                <span>씤洹 떆꽭</span>
                                <span className="text-[#cc5a27]">쁽옱 湲됰ℓ媛</span>
                            </div>
        ...
        >
            <div className="grid grid-cols-2 grid-rows-2 gap-8 h-[480px]">
                {[
                    { bg: 'bg-blue-50', border: 'border-blue-100', icon: '윆' },
                    { bg: 'bg-green-50', border: 'border-green-100', icon: '윆' },
                    { bg: 'bg-red-50', border: 'border-red-100', icon: '윋' },
                    { bg: 'bg-yellow-50', border: 'border-yellow-100', icon: '윆쀯툘' },
                ].map((style, i) => {
                    const idx = i + 1;
                    return (
            ...
            <div className="mt-8 text-right pr-4">
                <p className="text-gray-500 italic font-serif-kr text-lg">"理쒓퀬쓽 엯吏뿉 誘몃옒 媛移섎 뜑빀땲떎."</p>
            </div>
        </ReportPage>
        )}
    </div>
