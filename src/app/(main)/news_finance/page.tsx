...
export default async function NewsFinancePage() {
  const [articlesRes, popularRes, importantRes] = await Promise.all([
    ...
    getArticles({ status: "APPROVED", article_type: "IMPORTANT", section2: "부동산·주식·재테크", limit: 3 })
  ]);
  ...
  return <NewsListLayout category="부동산·주식·재테크" title="부동산·주식·재테크" initialArticles={articles} initialPopular={popular} importantArticles={importantArticles} />;
}
