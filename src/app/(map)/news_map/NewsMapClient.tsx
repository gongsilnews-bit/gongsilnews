...
export default function NewsMapClient({ initialArticles, initialPopularArticles }: { initialArticles: any[], initialPopularArticles: any[] }) {
  /* ── 상태 ── */
  const [allArticles, setAllArticles] = useState<any[]>(initialArticles);           // DB에서 불러온 전체 기사
  const [geoArticles, setGeoArticles] = useState<any[]>(initialArticles.filter(a => a.lat && a.lng));           // 좌표가 있는 기사만
  const [filteredArticles, setFilteredArticles] = useState<any[]>(initialArticles.filter(a => a.lat && a.lng));  // 현재 사이드바에 표시될 기사
  ...
  useEffect(() => {
    ...
    geoArticles.forEach(art => {
      ...
      const size = 32;
      const svgStr = `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}"><circle cx="${size/2}" cy="${size/2}" r="${size/2 - 2}" fill="%23ff8e15" stroke="white" stroke-width="2.5"/><text x="50%" y="50%" text-anchor="middle" dominant-baseline="central" fill="white" font-size="13" font-weight="bold" font-family="sans-serif">1</text></svg>`;
