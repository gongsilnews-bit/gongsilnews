...
export async function getMyArticles(authorId: string) {
  ...
  try {
    const { data, error } = await supabase
      .from("articles")
      .select("id, article_no, status, section1, section2, title, subtitle, author_name, author_id, published_at, created_at, updated_at, is_deleted, thumbnail_url, view_count, lat, lng, location_name, youtube_url, is_important, is_headline, reject_reason, edit_count")
      .eq("is_deleted", false)
      .eq("author_id", authorId)
      .order("published_at", { ascending: false, nullsFirst: false })
      .order("created_at", { ascending: false });
