...
import BoardWriteClient from "@/app/(main)/board_write/BoardWriteClient";

export default async function MobileBoardWritePage({
  searchParams,
}: {
  searchParams: Promise<{ [key: string]: string | undefined }>;
}) {
  const params = await searchParams;
  const boardId = params.board_id || "notice";
  ...
  // ?œë²„?ì„œ ? ì? ?•ë³´ ë¡œë“œ (?´ë¼?´ì–¸??ì¤‘ë³µ ?¸ì¶œ ë°©ì?)
  ...
  return (
    <div style={{ backgroundColor: '#fff', minHeight: '100vh', paddingBottom: '60px' }}>
      <Suspense fallback={<div style={{ padding: 40, textAlign: "center" }}>ë¶ˆëŸ¬?¤ëŠ” ì¤?..</div>}>
        <BoardWriteClient 
          board={board} 
