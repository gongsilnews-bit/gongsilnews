...
function MobileLoginClient() {
  ...
  const handleOAuthLogin = async (providerName: 'google' | 'kakao' | 'naver') => {
    ...
    try {
      ...
      // 성공 시 외부 리다이렉트로 페이지가 전환되므로 로딩 상태를 유지한다
    } catch (err: any) {
      console.error(err);
      ...
      {showFindAccount ? (
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            >
              {!oauthLoading && <span style={{ position: 'absolute', top: -9, right: 12, background: '#2563eb', color: '#fff', fontSize: 9, fontWeight: 900, padding: '2px 6px', borderRadius: 10 }}>추천</span>}
              {oauthLoading === 'google' ? (
                <>
