...
export default function MobileCommentPage() {
  ...
  const formatTime = (dateStr: string) => {
    ...
    if (diff < 86400000) return `${Math.floor(diff / 3600000)}?œê°„ ??;
    return `${d.getMonth() + 1}.${d.getDate()}`;
  };

  const isNew = (dateStr: string) => (new Date().getTime() - new Date(dateStr).getTime()) < 86400000;

  const typeLabel = (type: string) => type === "article" ? "ê¸°ì‚¬" : type === "vacancy" ? "ê³µì‹¤" : "?¹ê°•";
  ...
  const handleDelete = (id: string) => {
    if (!confirm("???“ê? ?Œë¦¼???? œ?˜ì‹œê² ìŠµ?ˆê¹Œ?")) return;
  ...
  return (
    <div style={{ minHeight: "100dvh", background: "#f4f5f7", fontFamily: "'Pretendard Variable', -apple-system, sans-serif" }}>
      ...
      <div style={{
        ...
        <h1 style={{ fontSize: 17, fontWeight: 800, color: "#111", margin: 0 }}>?“ê? ê´€ë¦?/h1>
        <span style={{ fontSize: 12, color: "#9ca3af", marginLeft: "auto" }}>
          ?„ì²´ {currentList.length}ê±?        </span>
      </div>

      {/* 2??*/}
      <div style={{ display: "flex", background: "#fff", borderBottom: "2px solid #e5e7eb" }}>
        {[
          { key: "myContent" as const, label: "??ê¸€ ?“ê?", count: myContentComments.filter(c => !hiddenIds.has(c.id)).length },
          { key: "myReplies" as const, label: "???“ê? ?µê?", count: myReplies.filter(c => !hiddenIds.has(c.id)).length },
        ].map(t => (
        ...
        {loading ? (
          ...
          <div style={{ padding: 50, textAlign: "center", color: "#9ca3af" }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 600 }}>
              {tab === "myContent" ? "??ê¸€???¬ë¦° ?“ê????†ìŠµ?ˆë‹¤" : "???“ê????µê????†ìŠµ?ˆë‹¤"}
            </div>
          </div>
        ) : currentList.map(c => (
          <div key={c.id} style={{
            ...
            <div style={{ fontSize: 14, color: "#111", lineHeight: 1.5, marginBottom: 6, wordBreak: "break-word" }}>
              {c.isSecret ? "?”’ ë¹„ë? ?“ê??…ë‹ˆ?? : c.content}
            </div>
            ...
            {tab === "myReplies" && (c as any).myOriginalComment && (
              <div style={{
                ...
                ???“ê?: {(c as any).myOriginalComment.length > 40 ? (c as any).myOriginalComment.slice(0, 40) + "..." : (c as any).myOriginalComment}
              </div>
            )}

            {/* ?ê? ?œëª© */}
            <div style={{ fontSize: 12, color: "#9ca3af", marginBottom: 10 }}>
              ?“Œ {c.sourceTitle.length > 35 ? c.sourceTitle.slice(0, 35) + "..." : c.sourceTitle}
            </div>
