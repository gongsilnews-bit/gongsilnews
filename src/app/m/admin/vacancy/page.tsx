...
function MobileVacancyAdmin() {
  ...
  useEffect(() => {
    async function init() {
      ...
      if (data) {
        setMemberId(data.id);
        setUserName(data.name || "?´ë¦„?†ìŒ");
        setUserPhone(data.phone || "");
        setUserRole(data.role);
        if (data.role === 'ADMIN' || data.role === 'SUPER_ADMIN' || data.role === 'ìµœê³ ê´€ë¦¬ì') {
          setIsAdmin(true);
        }
      }
      setAuthChecked(true);
  ...
  const filtered = vacancies.filter(v => {
    if (filter === "ê´‘ê³ ì¤? && v.status !== "ACTIVE") return false;
    if (filter === "ê´‘ê³ ì¢…ë£Œ" && v.status !== "STOPPED") return false;
    if (filter === "?„ì‹œ?€?? && v.status !== "DRAFT") return false;
    if (activeKeyword) {
  ...
  const formatAmount = (amt: number) => {
    ...
    if (r > 0) {
      ...
      if (e === 0 && c === 0 && rem > 0) result += "ë§?;
    }
    return result || "0";
  };

  const statusInfo: Record<string, { bg: string; label: string }> = {
    ACTIVE: { bg: "#10b981", label: "ê´‘ê³ ì¤? },
    STOPPED: { bg: "#ef4444", label: "ê´‘ê³ ì¢…ë£Œ" },
    DRAFT: { bg: "#9ca3af", label: "?„ì‹œ?€?? },
  };

  const tabs = [
    { key: "?„ì²´", count: vacancies.length },
    { key: "ê´‘ê³ ì¤?, count: vacancies.filter(v => v.status === "ACTIVE").length },
    { key: "ê´‘ê³ ì¢…ë£Œ", count: vacancies.filter(v => v.status === "STOPPED").length },
    { key: "?„ì‹œ?€??, count: vacancies.filter(v => v.status === "DRAFT").length },
  ];
    ...
    <div style={{ minHeight: "100dvh", background: "#f4f5f7", fontFamily: "'Pretendard Variable', -apple-system, sans-serif" }}>
      ...
      <div style={{ position: "sticky", top: 0, zIndex: 50, background: "#fff", borderBottom: "1px solid #e5e7eb", padding: "0 16px", height: 56, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <h1 style={{ fontSize: 18, fontWeight: 800, color: "#111", margin: 0 }}>ê³µì‹¤ê´€ë¦?/h1>
          <span style={{ fontSize: 12, color: "#6b7280", fontWeight: 600 }}>
            ê´‘ê³  {vacancies.filter(v => v.status === "ACTIVE").length}ê±?/ ?„ì²´ {vacancies.length}ê±?          </span>
        </div>
      ...
      {searchOpen && (
        <div style={{ background: "#fff", padding: "12px 16px", borderBottom: "1px solid #e5e7eb", display: "flex", gap: 8 }}>
          <input
            ...
            placeholder="ì£¼ì†Œ, ê±´ë¬¼ëª? ?±ë¡???ëŠ” ê³µì‹¤ë²ˆí˜¸ ê²€??
      ...
      <div style={{ background: "#fff", borderBottom: "1px solid #e5e7eb", padding: "0 12px", display: "flex", overflowX: "auto", WebkitOverflowScrolling: "touch" }} className="hide-scrollbar">
        {tabs.map(tab => (
          ...
          >
            {tab.key}
            <span style={{
              background: tab.key === "?„ì²´" ? "#e5e7eb" : tab.key === "ê´‘ê³ ì¤? ? "#10b981" : tab.key === "ê´‘ê³ ì¢…ë£Œ" ? "#ef4444" : tab.key === "?„ì‹œ?€?? ? "#9ca3af" : "#ef4444",
              color: tab.key === "?„ì²´" ? "#4b5563" : "#fff",
        ...
        {loading ? (
          ...
          <div style={{ padding: "60px 0", textAlign: "center", color: "#9ca3af" }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 600 }}>
              {filter === "?„ì²´" ? "?±ë¡??ê³µì‹¤???†ìŠµ?ˆë‹¤." : "ì¡°ê±´??ë§ëŠ” ê³µì‹¤???†ìŠµ?ˆë‹¤."}
            </div>
          </div>
        ) : filtered.map(row => {
          ...
          const priceText = row.trade_type === "ë§¤ë§¤" ? `ë§¤ë§¤ ${formatAmount(row.deposit)}`
            : row.trade_type === "?„ì„¸" ? `?„ì„¸ ${formatAmount(row.deposit)}`
            ...
            <div key={row.id} style={{
                  ...
                  {(row.status === "ACTIVE" || row.status === "STOPPED") ? (
                    <button
                      onClick={async () => {
                        ...
                        const msg = isActive ? "ê´‘ê³ ë¥?ì¢…ë£Œ?˜ì‹œê² ìŠµ?ˆê¹Œ?" : "ê´‘ê³ ë¥??œì‘?˜ì‹œê² ìŠµ?ˆê¹Œ?";
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
                ...
                <span style={{ fontSize: 13, color: "#6b7280" }}>
                  {row.room_count || "-"}ë°?/ {row.exclusive_m2 ? `${row.exclusive_m2}mÂ²` : "-"} / {row.current_floor || "-"}ì¸?                </span>
              </div>

              {/* ?±ë¡??+ ? ì§œ */}
              <div style={{ fontSize: 12, color: "#9ca3af", marginBottom: 12, display: "flex", gap: 12 }}>
                <span>{row.client_name || userName} Â· {row.client_phone || userPhone}</span>
                <span>{dateStr} ?±ë¡</span>
              </div>

              {/* ?¡ì…˜ ë²„íŠ¼ */}
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {/* ê¸°ë³¸ 3ì¢?ë²„íŠ¼ (ë¯¸ë¦¬ë³´ê¸°, ?˜ì •, ?? œ) - ??ƒ ?™ì¼???¬ê¸°ë¡???ì¤„ì— ?¸ì¶œ */}
                <div style={{ display: "flex", gap: 6 }}>
                  ...
                  <button onClick={async () => {
                    if (!confirm("??ê³µì‹¤???? œ?˜ì‹œê² ìŠµ?ˆê¹Œ?")) return;
                ...
                {/* AI ?¨ë¼???„ë‹¨ì§€ ?„ìš© ê³µìœ  / ë¹„í™œ??ë²„íŠ¼ */}
                {(userRole === 'ADMIN' || userRole === 'SUPER_ADMIN' || userRole === 'ìµœê³ ê´€ë¦¬ì' || userRole === 'ë¶€?™ì‚°' || userRole === 'REALTOR') && (() => {
                  ...
                  if (hasFlyer) {
                      ...
                      <button 
                        onClick={() => {
                          ...
                          navigator.clipboard.writeText(shareUrl).then(() => {
                            alert("?¨ë¼?¸ì „?¨ì? ë§í¬ ì£¼ì†Œê°€ ë³µì‚¬?˜ì—ˆ?µë‹ˆ??\n?í•˜???€?”ë°©??ë¶™ì—¬?£ì–´ ?„ì†¡?´ë³´?¸ìš”!");
                          }).catch(() => {
                    ...
                    return (
                      <button 
                        onClick={() => {
                          alert("??ë§¤ë¬¼?€ ?„ì§ AI ?¨ë¼?¸ì „?¨ì?ê°€ ?œì‘?˜ì? ?Šì•˜?µë‹ˆ??\nPC ë²„ì „ ê³µì‹¤ê´€ë¦¬ì—???„ë‹¨ì§€ë¥?ë¨¼ì? ?œì‘/?€?¥í•´ ì£¼ì„¸??");
                        }}
                        style={{ 
                          width: "100%", 
                          height: 38, 
