...
function MobileMemberWrite() {
  ...
  const handleApprove = async (status: "APPROVED" | "REJECTED") => {
    ...
    if (!confirm(`???Œì›??${isApprove ? '?¹ì¸' : 'ë°˜ë ¤'} ì²˜ë¦¬?˜ì‹œê² ìŠµ?ˆê¹Œ?`)) return;
    ...
    if (isApprove) {
      ...
      const reason = prompt("ë°˜ë ¤ ?¬ìœ ë¥??…ë ¥?´ì£¼?¸ìš” (ì·¨ì†Œ ??'?œë¥˜ ë¶ˆì¼ì¹?ë¡??ë™ ?…ë ¥?©ë‹ˆ??:") || "?œë¥˜ ë¶ˆì¼ì¹?;
      res = await adminRejectRealtorApplication(member.id, reason);
    }

    if (res.success) {
      alert(`${isApprove ? '?¹ì¸' : 'ë°˜ë ¤'} ì²˜ë¦¬?˜ì—ˆ?µë‹ˆ??`);
      router.back();
    } else {
      alert("ì²˜ë¦¬ ì¤??¤ë¥˜ê°€ ë°œìƒ?ˆìŠµ?ˆë‹¤: " + res.error);
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return (
      <div style={{ display: "flex", height: "100dvh", alignItems: "center", justifyContent: "center", background: "#f4f5f7" }}>
        <div style={{ color: "#9ca3af", fontWeight: 600 }}>ë¶ˆëŸ¬?¤ëŠ” ì¤?..</div>
      </div>
    );
  }
  ...
  const roleMap: any = { 'ADMIN': 'ìµœê³ ê´€ë¦¬ì', 'REALTOR': 'ë¶€?™ì‚°?Œì›', 'USER': '?¼ë°˜?Œì›' };
  const roleName = roleMap[member.role] || member.role || '?¼ë°˜?Œì›';
  ...
  return (
    <div style={{ minHeight: "100dvh", background: "#f4f5f7", fontFamily: "'Pretendard Variable', -apple-system, sans-serif", paddingBottom: "80px" }}>
      ...
      <div style={{ position: "sticky", top: 0, zIndex: 50, background: "#fff", borderBottom: "1px solid #e5e7eb", padding: "0 16px", height: 56, display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <h1 style={{ fontSize: 18, fontWeight: 800, color: "#111", margin: 0 }}>?Œì› ?ì„¸ ë°?ì²˜ë¦¬</h1>
      </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
            ...
            <div style={{ position: "relative", display: "inline-flex", alignItems: "center" }}>
              ...
              >
                ...
                <option value="REALTOR">ë¶€?™ì‚°?Œì›</option>
            ...
            {member.role === 'REALTOR' && agency && (
              <span style={{ fontSize: 11, fontWeight: 700, padding: "2px 6px", borderRadius: 4, background: isPending ? "#fef3c7" : isApproved ? "#d1fae5" : "#fee2e2", color: isPending ? "#92400e" : isApproved ? "#065f46" : "#b91c1c" }}>
                {isPending ? "?¹ì¸?€ê¸? : isApproved ? "?•ìƒ?¹ì¸" : "?œë¥˜ë³´ì™„"}
              </span>
            )}
          </div>
      ...
      <div style={{ display: "flex", background: "#fff", borderBottom: "1px solid #e5e7eb" }}>
        <button onClick={() => setActiveTab("basic")} style={{ flex: 1, height: 48, background: "none", border: "none", borderBottom: activeTab === "basic" ? "2px solid #111" : "2px solid transparent", color: activeTab === "basic" ? "#111" : "#6b7280", fontSize: 14, fontWeight: activeTab === "basic" ? 800 : 600, cursor: "pointer" }}>ê¸°ë³¸ ?•ë³´</button>
        {member.role === 'REALTOR' && (
          <button onClick={() => setActiveTab("agency")} style={{ flex: 1, height: 48, background: "none", border: "none", borderBottom: activeTab === "agency" ? "2px solid #111" : "2px solid transparent", color: activeTab === "agency" ? "#111" : "#6b7280", fontSize: 14, fontWeight: activeTab === "agency" ? 800 : 600, cursor: "pointer" }}>ë¶€?™ì‚°/?œë¥˜</button>
        )}
      </div>
      ...
      <div style={{ padding: "16px" }}>
        {activeTab === "basic" && (
          <div style={{ background: "#fff", borderRadius: 12, padding: "16px", boxShadow: "0 1px 3px rgba(0,0,0,0.05)" }}>
            {renderInfoRow("?Œì›ë²ˆí˜¸", member.memberNumber ? `#${member.memberNumber}` : "-")}
            {renderInfoRow("?°ë½ì²?, member.phone)}
            {renderInfoRow("ê°€?…ì¼", member.created_at ? new Date(member.created_at).toLocaleDateString() : "-")}
            {member.role === 'REALTOR' && (
              <>
                {renderInfoRow("?”ê¸ˆ??, member.plan_type === 'news_premium' ? 'ê³µì‹¤?´ìŠ¤ë¶€?™ì‚°' : member.plan_type === 'vacancy_premium' ? 'ê³µì‹¤?±ë¡ë¶€?™ì‚°' : 'ë¬´ë£Œë¶€?™ì‚°')}
                {renderInfoRow("ìµœë? ê³µì‹¤ê´‘ê³ ", `${member.max_vacancies || 5}ê°?)}
                {renderInfoRow("??ê¸°ì‚¬", `${member.max_articles_per_month || 0}ê°?)}
              </>
            )}
          </div>
        )}
        ...
        {activeTab === "agency" && agency && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ background: "#fff", borderRadius: 12, padding: "16px", boxShadow: "0 1px 3px rgba(0,0,0,0.05)" }}>
              <h3 style={{ fontSize: 15, fontWeight: 800, color: "#111", margin: "0 0 12px 0" }}>ì¤‘ê°œ?…ì†Œ ?•ë³´</h3>
              {renderInfoRow("?í˜¸ëª?, agency.name)}
              {renderInfoRow("?€?œìëª?, agency.ceo_name)}
              {renderInfoRow("?´ë???, agency.cell)}
              {renderInfoRow("?¬ë¬´?¤ë²ˆ??, agency.phone)}
              {renderInfoRow("ì£¼ì†Œ", `[${agency.zipcode || '-'}] ${agency.address || ''} ${agency.address_detail || ''}`)}
              {renderInfoRow("?±ë¡ë²ˆí˜¸", agency.reg_num)}
              {renderInfoRow("?¬ì—…?ë²ˆ??, agency.biz_num)}
              {renderInfoRow("?Œê°œ", agency.intro)}
            </div>

            <div style={{ background: "#fff", borderRadius: 12, padding: "16px", boxShadow: "0 1px 3px rgba(0,0,0,0.05)" }}>
              <h3 style={{ fontSize: 15, fontWeight: 800, color: "#111", margin: "0 0 12px 0" }}>ì²¨ë? ?œë¥˜</h3>
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                <div style={{ flex: 1, minWidth: "140px" }}>
                  <div style={{ fontSize: 12, color: "#6b7280", fontWeight: 600, marginBottom: 8 }}>?¬ì—…?ë“±ë¡ì¦</div>
                  <div 
                  ...
                  >
                    {agency.biz_cert_url ? (
                      <img src={agency.biz_cert_url} alt="?¬ì—…?ë“±ë¡ì¦" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                    ) : (
                      <span style={{ fontSize: 12, color: "#9ca3af" }}>ë¯¸ë“±ë¡?/span>
                    )}
                  </div>
                </div>
                <div style={{ flex: 1, minWidth: "140px" }}>
                  <div style={{ fontSize: 12, color: "#6b7280", fontWeight: 600, marginBottom: 8 }}>ì¤‘ê°œ?…ë“±ë¡ì¦</div>
                  <div 
                  ...
                  >
                    {agency.reg_cert_url ? (
                      <img src={agency.reg_cert_url} alt="ì¤‘ê°œ?…ë“±ë¡ì¦" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                    ) : (
                      <span style={{ fontSize: 12, color: "#9ca3af" }}>ë¯¸ë“±ë¡?/span>
                    )}
                  </div>
              ...
              <div style={{ fontSize: 11, color: "#9ca3af", marginTop: 8 }}>* ?´ë?ì§€ë¥???•˜ë©??¬ê²Œ ë³????ˆìŠµ?ˆë‹¤.</div>
            </div>
          </div>
        )}
      </div>
