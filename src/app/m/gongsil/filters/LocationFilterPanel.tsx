  ...
  if (variant === "inline") {
    return (
      <div style={{ position: "relative", zIndex: isExpanded ? 10020 : 1 }}>
        {/* ?„ì²´ ?”ë©´????Š” ê²€?•ìƒ‰ ë°°ê²½ ?¤ë²„?ˆì´ (ë²„íŠ¼ê³??ì—…?€ ???„ì— ?œì‹œ?? */}
        {isExpanded && (
          <div 
        ...
        {/* ?´ë¦­??ê³??„ë˜???ˆì°½(Popover) ?•íƒœë¡??¨ëŠ” ì§€??? íƒì°?*/}
        {isExpanded && (
          <div style={{ position: "absolute", top: "100%", left: 0, right: 0, marginTop: "10px", zIndex: 2, background: "#fff", borderRadius: "14px", border: "1px solid #e5e7eb", boxShadow: "0 10px 30px rgba(0,0,0,0.15)", padding: "16px", animation: "fadeIn 0.2s" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
              <span style={{ fontSize: "15px", fontWeight: 800, color: "#111" }}>
                {regTab === "sido" ? "????? íƒ" : regTab === "gugun" ? `${selSido} ?˜ìœ„ ì§€?? : `${selGugun} ?˜ìœ„ ì§€??}
              </span>
            ...
            <div className="no-scrollbar" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "8px", maxHeight: "240px", overflowY: "auto", paddingRight: "4px" }}>
              ...
              {regTab === "gugun" && (!selSidoCode ? <div style={{ gridColumn: "1/-1", textAlign: "center", padding: "20px", color: "#9ca3af" }}>???„ë? ë¨¼ì? ? íƒ?˜ì„¸??/div> : gugunList.length > 0 ? gugunList.map(c => (
                <button key={c.code} onClick={() => { 
              ...
              {regTab === "dong" && (!selGugunCode ? <div style={{ gridColumn: "1/-1", textAlign: "center", padding: "20px", color: "#9ca3af" }}>??êµ?êµ¬ë? ë¨¼ì? ? íƒ?˜ì„¸??/div> : dongList.length > 0 ? dongList.map(c => (
                <button key={c.code} onClick={() => { 
  ...
  return (
    <div>
      <div style={{ display: "flex", borderBottom: "2px solid #f3f4f6", marginBottom: "16px" }}>
        <button onClick={() => setLocTab("region")} style={{ flex: 1, padding: "10px", fontSize: "14px", fontWeight: locTab === "region" ? 700 : 500, color: locTab === "region" ? "#4b89ff" : "#9ca3af", borderBottom: locTab === "region" ? "2px solid #4b89ff" : "2px solid transparent", background: "none", border: "none", cursor: "pointer" }}>ì§€??„ ??/button>
        <button onClick={() => setLocTab("keyword")} style={{ flex: 1, padding: "10px", fontSize: "14px", fontWeight: locTab === "keyword" ? 700 : 500, color: locTab === "keyword" ? "#4b89ff" : "#9ca3af", borderBottom: locTab === "keyword" ? "2px solid #4b89ff" : "2px solid transparent", background: "none", border: "none", cursor: "pointer" }}>?¤ì›Œ?œê???/button>
      </div>
      ...
      {locTab === "region" ? (
        ...
        <div>
          <div style={{ display: "flex", gap: "8px", marginBottom: "12px" }}>
            ...
            <button onClick={doLocKeywordSearch} style={{ padding: "10px 16px", background: "#4b89ff", color: "#fff", border: "none", borderRadius: "8px", fontWeight: 700, fontSize: "14px", cursor: "pointer" }}>?´ë™</button>
          </div>
