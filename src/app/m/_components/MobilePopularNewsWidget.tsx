...
function MobilePopularNewsWidgetImpl({
  ...
  const popularArticles = useMemo(() => {
    ...
    if (section2Tab && section2Tab !== "전체") {
      pool = pool.filter((a) => a.section2 === section2Tab);
    }
    ...
    // 3. 조회수(view_count) 내림차순 정렬 (동점 시 최신순)
    ...
    // 4. 안전망(Fallback): 해당 기간 기사가 5개 미만이면 해당 카테고리 기사로 보충
    if (top5.length < 5) {
      let subPool = allArticles;
      if (section2Tab && section2Tab !== "전체") {
        subPool = subPool.filter((a) => a.section2 === section2Tab);
      }
    ...
    return top5;
  }, [allArticles, section2Tab, period, customStartDate, customEndDate]);
  ...
  return (
    <div style={{ padding: "20px 16px", borderBottom: "8px solid #f4f6f8", backgroundColor: "#fff", position: "relative" }}>
      {/* 타이틀 + 기간 셀렉트박스 (전체 1줄 구분선) */}
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px", paddingBottom: "8px", borderBottom: "1px solid #f3f4f6" }}>
        <div style={{ display: "flex", alignItems: "center", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1, marginRight: "8px" }}>
          <span style={{ fontSize: "16px", fontWeight: 800, color: "#508bf5", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            {displayCategoryName}
          </span>
          <span style={{ fontSize: "16px", fontWeight: 800, color: "#1f2937", marginLeft: "5px", flexShrink: 0 }}>
            많이 본 뉴스
          </span>
        </div>
      ...
      {showCustomModal && (
        ...
        >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
            <div>
              <label style={{ fontSize: "12px", color: "#6b7280", display: "block", marginBottom: "3px" }}>시작일</label>
              <input
                type="date"
            ...
            <div>
              <label style={{ fontSize: "12px", color: "#6b7280", display: "block", marginBottom: "3px" }}>종료일</label>
              <input
                type="date"
                value={customEndDate}
