...
function MobileBottomNavContent() {
  ...
  // ?§ÌÅ¨Î°?Î∞òÏùë???òÎã®Î∞??ÅÌÉú
  const [isVisible, setIsVisible] = useState(true);
  const lastScrollY = React.useRef(0);

  // ÏßÄ???òÏù¥ÏßÄ ?êÎäî ?∞Î¶¨?ôÎÑ§?¥Ïä§ ??óê?úÎäî ??ÉÅ ?òÎã®Î∞îÍ? Î≥¥Ïù¥?ÑÎ°ù Î¶¨ÏÖã
  useEffect(() => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // ?§ÌÅ¨Î°§ÏùÑ ?¥Î¶¥ ??(?ÑÏû¨ ?§ÌÅ¨Î°§Ïù¥ ?¥Ï†ÑÎ≥¥Îã§ ?¨Í≥†, ?ÅÎã®?êÏÑú 50px ?¥ÏÉÅ ?¥Î†§?îÏùÑ ??
  ...
  // Í≥µÏã§?±Î°ù/?òÏ†ï ?òÏù¥ÏßÄ?êÏÑú???ÑÏó≠ ??∞î ?®Í? (???ÑÏö© BottomNav ?¨Ïö©)
  ...
  const navItems = [
    ...
    {
      name: "Î∂Ä?ôÏÇ∞?πÍ∞ï", path: "/m/study",
      ...
      >
        {navItems.map((item) => {
          ...
          const activeColor = isActive
            ? ((item.name === "?? || item.name === "Í≥µÏã§") ? '#1a73e8' : (item.name === "Î∂Ä?ôÏÇ∞?πÍ∞ï" ? '#1a2e50' : '#ea580c'))
            : '#333';

          return item.name === "ÎßàÏù¥" ? (
            <Link
              key={item.name}
              href="/m/menu"
