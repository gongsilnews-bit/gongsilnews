...
function formatPrice(v: any): string {
  ...
  if (v.trade_type === 'ê²½ë§¤') return formatValue(v.deposit || 0);
  if (v.trade_type === 'ë§¤ë§¤') return formatValue(v.sale_price || v.deposit || 0);
  if (v.trade_type === '?„ì„¸') return formatValue(v.deposit || 0);
  if (v.trade_type === '?”ì„¸') {
...
const CATEGORIES = [
  { key: "news_gongsil", label: "ê³µì‹¤?´ìŠ¤", path: "/m/news_gongsil", section1: "ê³µì‹¤?´ìŠ¤" },
  { key: "news_politics", label: "ë¶€?™ì‚°Â·ê²½ì œ", path: "/m/news_politics", section1: "ë¶€?™ì‚°Â·ê²½ì œ" },
  { key: "news_marketing", label: "AIë§ˆì???, path: "/m/news_marketing", section1: "AIë§ˆì??? },
  { key: "news_etc", label: "?¼ì´?„Â·ì˜¤?¼ë‹ˆ??, path: "/m/news_etc", section1: "?¼ì´?„Â·ì˜¤?¼ë‹ˆ?? },
];

const KEY_TO_SECTION1: Record<string, string> = {
  "news_gongsil": "ê³µì‹¤?´ìŠ¤",
  "news_politics": "ë¶€?™ì‚°Â·ê²½ì œ",
  "news_marketing": "AIë§ˆì???,
  "news_etc": "?¼ì´?„Â·ì˜¤?¼ë‹ˆ??
};

const SECTION1_TO_KEY: Record<string, string> = {
  "ê³µì‹¤?´ìŠ¤": "news_gongsil",
  "ë¶€?™ì‚°Â·ê²½ì œ": "news_politics",
  "AIë§ˆì???: "news_marketing",
  "?¼ì´?„Â·ì˜¤?¼ë‹ˆ??: "news_etc"
};

// PC?€ ?™ì¼??2ì°?ì¹´í…Œê³ ë¦¬ ë§?
const SECTION2_MAP: Record<string, string[]> = {
  "ê³µì‹¤?´ìŠ¤": ["?„íŒŒ???¤í”¼?¤í…”", "ë¹Œë¼/ì£¼íƒ", "?ë£¸/?¬ë£¸(?€?µì…˜)", "?ê?/?¬ë¬´??ê³µì¥/? ì?", "? ì¶•/ë¶„ì–‘/ê²½ë§¤"],
  "ë¶€?™ì‚°Â·ê²½ì œ": ["ë¶€?™ì‚° ?•ì±…/?™í–¥", "ê²½ì œ/?¬í…Œ??ì£¼ì‹", "ë²•ë¥ /?¸ë¬´ ì§€??],
  "AIë§ˆì???: ["AI/NEWS", "ë¶€?™ì‚°? íŠœë¸?ë¸”ë¡œê·?, "ê³µì‹¤/?„ë?ê´€ë¦?],
  "?¼ì´?„Â·ì˜¤?¼ë‹ˆ??: ["?¸ë¬¼/?¸í„°ë·?, "ë¶€?™ì‚°/?¸í…Œë¦¬ì–´ ê¿€??, "ë§›ì§‘/?¬í–‰/ê±´ê°•", "?ìœ  ?ì„¸??],
};
...
const PERSONALIZED_MENTAL_MAP: Record<string, Record<string, string>> = {
  "news_gongsil": {
    "?„ì²´": "?¤ì‹œê°?ì¤‘ê°œ??ê³µì‹¤ ?Œì‹",
    "?„íŒŒ???¤í”¼?¤í…”": "ê³µë™ì¤‘ê°œ ì¶”ì²œ ?„íŒŒ?¸Â·ì˜¤?¼ìŠ¤??,
    "ë¹Œë¼/ì£¼íƒ": "ê³„ì•½ ?•ë¥  ?’ì? ë¹Œë¼Â·ì£¼íƒ ë§¤ë¬¼",
    "?ë£¸/?¬ë£¸(?€?µì…˜)": "?ë£¸Â·?¬ë£¸ ?¤ë¬´ ?¸ë Œ??,
    "?ê?/?¬ë¬´??ê³µì¥/? ì?": "ê³ ìˆ˜???ê?Â·?¬ë¬´???¤ë¬´ ?•ë³´",
    "? ì¶•/ë¶„ì–‘/ê²½ë§¤": "?¨ê¸° ì°¨ìµ ? ì¶•Â·ë¶„ì–‘Â·ê²½ë§¤ ?´ìŠ¤"
  },
  "news_politics": {
    "?„ì²´": "ê³ ê° ë¸Œë¦¬?‘ìš© ?¤ëŠ˜???œì¥ ?™í–¥",
    "ë¶€?™ì‚° ?•ì±…/?™í–¥": "?ë‹´ ?„ìˆ˜ ?•ì±… ë¶„ì„ & ê·œì œ ?™í–¥",
    "ê²½ì œ/?¬í…Œ??ì£¼ì‹": "ê±°ì‹œê²½ì œÂ·?¬í…Œ??ë°”ì´ë¸?,
    "ë²•ë¥ /?¸ë¬´ ì§€??: "ê³ ê°??ë¬»ê¸° ?„ì— ?€ë¹„í•˜???¸ë¬´Â·ë²•ë¥  ?”ë£¨??
  },
  "news_marketing": {
    "?„ì²´": "ë§¤ë¬¼ ë¬¸ì˜ ??°œ?˜ëŠ” ë§ˆì???ë¹„ë²•",
    "AI/NEWS": "?…ë¬´ ?œê°„???ˆë°˜?¼ë¡œ ì¤„ì—¬ì¤?AI ?œìš©ë²?,
    "ë¶€?™ì‚°? íŠœë¸?ë¸”ë¡œê·?: "ì§€??1??ì¤‘ê°œ?…ì†Œ ë¸”ë¡œê·¸Â·ìœ ?œë¸Œ ê³µëµë²?,
    "ê³µì‹¤/?„ë?ê´€ë¦?: "?¨ìœ¨?ì¸ ê³µì‹¤Â·?„ë?ê´€ë¦??¸í•˜??
  },
  "news_etc": {
    "?„ì²´": "?¼ì˜ ë³´ëŒê³??±ê³µ???”í•´ì¤??¤í† ë¦?,
    "?¸ë¬¼/?¸í„°ë·?: "?µë? ?°ë´‰ ì¤‘ê°œ?¬ë“¤???¤ì „ ?±ê³µ ?¸í„°ë·?,
    "ë¶€?™ì‚°/?¸í…Œë¦¬ì–´ ê¿€??: "?¤ì „ ê³µê°„/?¸í…Œë¦¬ì–´ ?¸í•˜??,
    "ë§›ì§‘/?¬í–‰/ê±´ê°•": "?„ì¥ ?œë™??ë§ì? ?€?œë‹˜ ?„ìš© ê±´ê°• ë°”ì´ë¸?,
    "?ìœ  ?ì„¸??: "?¼ìƒ???¼í‘œ, ê°ì„± ?ì„¸??
  }
};
    ...
    <div style={{ padding: "20px 0", borderBottom: "8px solid #f4f6f8" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "16px", padding: "0 16px 8px 16px", borderBottom: "1px solid #f3f4f6" }}>
        <div style={{ display: "flex", alignItems: "center" }}>
          ...
          <span style={{ fontSize: "16px", fontWeight: 800, color: "#1f2937", marginLeft: "5px" }}>ì¶”ì²œ ?´ìŠ¤</span>
        </div>
            ...
            <div style={{ width: "100%", aspectRatio: "4/3", position: "relative", backgroundColor: "#111", borderRadius: "16px", overflow: "hidden", boxShadow: "0 4px 12px rgba(0,0,0,0.1)" }}>
              ...
              <div style={{ position: "absolute", bottom: 0, left: 0, width: "100%", padding: "20px 16px", zIndex: 3, display: "flex", flexDirection: "column", gap: "8px" }}>
                ...
                <div style={{ fontSize: "12px", color: "#ddd", display: "flex", alignItems: "center", gap: "8px" }}>
                  ...
                  <span>Â·</span>
                  <span>{art.author_name || "ê³µì‹¤?´ìŠ¤"}</span>
                </div>
              </div>
    ...
    >
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0, justifyContent: "center" }}>
        <div style={{ fontSize: "13px", display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "space-between", gap: "6px", marginBottom: "6px" }}>
          <span style={{ color: "#508bf5", fontWeight: 700 }}>{a.section2 || "?´ìŠ¤"}</span>
          <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
            <span style={{ color: "#666", fontWeight: 500 }}>
              {formatDate(a.published_at || a.created_at)} Â· {a.author_name || "ê³µì‹¤?´ìŠ¤"}
            </span>
...
function MobileNewsClient({ initialTab, initialArticles, initialAuthorName, initialKeyword, authorProfile }: { initialTab: string, initialArticles: any[], initialAuthorName?: string, initialKeyword?: string, authorProfile?: any }) {
  ...
  // ???„í™˜ ???•ë ¬ ê¸°ì???'ìµœì‹ ???¼ë¡œ ì´ˆê¸°??
  useEffect(() => {
    setSortBy('newest');
  }, [activeTab]);

  // Restore scroll position from sessionStorage on mount of MobileNewsClient
  // scrollTo ê°€ë¡œì±„ê¸? Next.js ?¼ìš°?°ì˜ scrollTo(0) ?¸ì¶œ???¼ì‹œ ì°¨ë‹¨?˜ì—¬ ê¹œë¹¡??flicker) ?„ì „ ?œê±°
  useLayoutEffect(() => {
    ...
    if (savedScroll) {
      ...
      // 2. Next.js ?¼ìš°?°ì˜ scrollTo(0) ?¸ì¶œ???¼ì‹œ ì°¨ë‹¨
      ...
      (window as any).scrollTo = function(...args: any[]) {
        ...
        if (targetY === 0) return; // Next.js??scroll-to-top ì°¨ë‹¨
        return origScrollTo.apply(window, args as any);
      };

      // 3. Next.js ?¼ìš°??ì²˜ë¦¬ ?„ë£Œ ???ë³¸ scrollTo ë³µì› (300msë©?ì¶©ë¶„)
      setTimeout(() => {
  ...
  // URL ?Œë¼ë¯¸í„°ê°€ ë³€ê²½ë˜ë©??íƒœ ?™ê¸°??(?¤ë¡œê°€ê¸???ë³µêµ¬??
  useEffect(() => {
    setSection2Tab(searchParams.get("section2") || "");
  }, [searchParams]);

  // 2ì°????´ë¦­ ???íƒœ ë³€ê²?ë°?URL ?…ë°?´íŠ¸ (history???€?¥ë˜???¤ë¡œê°€ê¸???ë³µêµ¬ ê°€??
  ...
  // ê¸°ì‚¬ ?ì„¸ ?¨ë„ ?¤ë¡œê°€ê¸?ì²˜ë¦¬ (showDetail ? ì–¸ ?´í›„??ë°°ì¹˜)
  // ê¸°ì‚¬ ?ì„¸/ë¦¬ìŠ¤???¨ë„ ?¤ë¡œê°€ê¸?ì²˜ë¦¬
  useEffect(() => {
  ...
  // ? ë‹ˆë©”ì´???¤ë²„?ˆì´ ?íƒœ???„ì „???œê±°??(ì¦‰ê°?ì¸ ?”ë©´ ?„í™˜???„í•´)
  // URL????´ ë³€ê²½ë˜ë©?activeTab ?íƒœë¥??™ê¸°??(êµ¬ë²„???Œë¼ë¯¸í„° ë¡œì§ ?œê±°)
  useEffect(() => {
  ...
  const [memberName, setMemberName] = useState<string>("ë¶€?™ì‚°");
  ...
  // URL ?Œë¼ë¯¸í„°ê°€ ë³€ê²½ë˜ë©?ì§€???„í„°???™ê¸°??(?¤ë¡œê°€ê¸??„í™˜??
  useEffect(() => {
  ...
  // ?¹ì…˜ ?„í„°ê°€ ?ìš©??localArticles (ì§€??ë§ˆì»¤ + ë¦¬ìŠ¤??ëª¨ë‘???¬ìš©)
  ...
  // ?¹ì…˜ ?„í„° ?ìš© (section1/section2 ?•í™• ë§¤ì¹­)
  ...
  // ?¼ë°˜ ?´ìŠ¤ ë¡œë“œ (?œë²„?ì„œ ë°›ì? initialArticles ìµœìš°???¬ìš©)
  useEffect(() => {
  ...
  // ???„í™˜ ???´ë‹¹ ì¹´í…Œê³ ë¦¬ ê¸°ì‚¬ë¥??´ë¼?´ì–¸?¸ì—??ì§ì ‘ fetch (SPA ?„í™˜?¼ë¡œ ?œë²„ ì»´í¬?ŒíŠ¸ê°€ ???????€ë¹?
  useEffect(() => {
  ...
  // ?¤ë¡œ ê°€ê¸??ˆë“œë¡œì´???˜ë“œ?¨ì–´ ë°±ë²„???? ì²˜ë¦¬ - pushStateë¡??µì¼?˜ì—¬ Next.js ?¼ìš°??ì¶©ëŒ ë°©ì?
  // popstate ?¸ë“¤?¬ëŠ” ?„ì˜ ?µí•© useEffect?ì„œ ì²˜ë¦¬

  // ê¸°ì‚¬ ?ì„¸ ë³€ê²????¤í¬ë¡?ìµœìƒ??ê°•ì œ ì´ˆê¸°??(ê°€???•ì‹¤??ë°©ë²•)
  useEffect(() => {
    if (showDetail && detailPanelRef.current) {
      const el = detailPanelRef.current;
      el.scrollTop = 0;

      // ?¹ì‹œ ëª¨ë? ?Œë”ë§??œë ˆ?´ë? ?€ë¹„í•´ ?¬ëŸ¬ ë²?ê°•ì œ ì´ˆê¸°??
      let attempts = 0;
  ...
  const handleSelectArticle = async (id: string, isLocal: boolean = false, e?: React.MouseEvent) => {
    if (isLocal) {
      ...
      // 2. ë°±ê·¸?¼ìš´?œì—???ì„¸ ?°ì´??ë³´ì™„ (ì¡°íšŒ??ì¦ê?, ?“ê?, ?¤ì›Œ????
      const res = await getArticleDetail(id);
      if (res.success && res.data) {
        setArticleDetail((prev: any) => {
          if (prev && prev.id === res.data.id && prev.content?.includes("iframe")) {
            // ? íŠœë¸???iframe???¬í•¨??ê²½ìš°, ?Œë”ë§?ê¹œë¹¡???ìƒ ?Šê?)??ë°©ì??˜ê¸° ?„í•´ ê¸°ì¡´ content ? ì?
        ...
        const check = setInterval(() => {
          if ((window as any).kakao?.maps?.LatLng) {
            ...
            if ((window as any).kakao.maps.MarkerClusterer) {
              ...
              setTimeout(() => clearInterval(check2), 5000); // 5ì´??€?„ì•„??
            }
          }
        }, 100);
      }
  ...
  // ì§€?„ì— ë§ˆì»¤ ?…ë°?´íŠ¸ (localArticles ë³€ê²???
  useEffect(() => {
  ...
  // ?€?€ ?¤ì??´í”„(ì¢Œìš° ?¬ë¼?´ë“œ) ê¸°ëŠ¥???œê±°?˜ê³  ?ë‹¨ ???´ë¦­ ë°©ì‹?¼ë¡œ ?¨ìˆœ?”ë˜?ˆìŠµ?ˆë‹¤ ?€?€
  const tabBarRef = useRef<HTMLDivElement>(null);

  // ??ë³€ê²???ì¹´í…Œê³ ë¦¬ ë°??ë™ ?¤í¬ë¡?
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      {/* ì¹´í…Œê³ ë¦¬ ??°” */}
          {/*
            ?¸ë? wrapper: position fixed + borderBottom ?¬ê¸°?ë§Œ ì§€?????Œìƒ‰ ë°??ˆë? ???¬ë¼ì§?
            ?´ë? ?ì‹?ëŠ” border ?†ìŒ
          */}
          <div
            style={{
      ...
      {/* ?°ë¦¬?™ë„¤?´ìŠ¤: ì¹´ì¹´??ì§€??+ ëª©ë¡ ?¤í”Œë¦?ë·?(?¤ì›Œ??ê²€?‰ì´???‘ê? ê²€?‰ì´ ?„ë‹ ?Œë§Œ ì§€??ë·°ë¡œ ?Œë”ë§? */}
      {(activeTab === "local" && !searchParams.get("keyword") && !searchParams.get("author_name") && !initialKeyword && !initialAuthorName) ? (
        <div style={{ position: "fixed", top: "56px", bottom: "60px", left: "0", right: "0", margin: "0 auto", width: "100%", maxWidth: "448px", display: "flex", flexDirection: "column", zIndex: 10, background: "#fff" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", background: "#fff", borderBottom: "1px solid #e5e7eb", padding: "8px 0", flexShrink: 0, width: "100%" }}>
              <div style={{ position: "relative", flex: 1, minWidth: 0, overflow: "hidden" }}>
                <div className="news-filter-scroll" onTouchStart={(e) => e.stopPropagation()} onTouchEnd={(e) => e.stopPropagation()} style={{ overflowX: "auto", display: "flex", gap: "8px", padding: "0 12px", WebkitOverflowScrolling: "touch" as any, alignItems: "center" }}>
                  <button onClick={() => setLocActivePanel(locActivePanel === "loc" ? null : "loc")} style={{ padding: "7px 14px", borderRadius: "20px", fontSize: "13px", fontWeight: 600, whiteSpace: "nowrap", flexShrink: 0, border: (locActivePanel === "loc" || locLabel !== "?„ì¹˜") ? "1.5px solid #508bf5" : "1px solid #d1d5db", background: (locActivePanel === "loc" || locLabel !== "?„ì¹˜") ? "#f0f6ff" : "#fff", color: (locActivePanel === "loc" || locLabel !== "?„ì¹˜") ? "#508bf5" : "#374151", cursor: "pointer", transition: "all 0.15s", display: "flex", alignItems: "center", gap: "4px" }}>
                    ?“ {locLabel} ??
                  </button>
              ...
              <>
                ...
                <div style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 448, background: "#fff", borderRadius: "16px 16px 0 0", zIndex: 9991, maxHeight: "55vh", display: "flex", flexDirection: "column", animation: "newsSheetUp 0.3s ease-out" }}>
                  <div style={{ padding: "16px 20px 12px", borderBottom: "1px solid #f3f4f6", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <span style={{ fontSize: "16px", fontWeight: 800, color: "#111" }}>?“ ?„ì¹˜ ê²€??/span>
                  ...
                  <div style={{ flex: 1, overflowY: "auto", padding: "16px 20px 24px", WebkitOverflowScrolling: "touch", overscrollBehaviorY: "contain" }}>
                    ...
                    <div style={{ display: "flex", borderBottom: "2px solid #f3f4f6", marginBottom: "16px" }}>
                      <button onClick={() => setLocTab("region")} style={{ flex: 1, padding: "10px", fontSize: "14px", fontWeight: locTab === "region" ? 700 : 500, color: locTab === "region" ? "#508bf5" : "#9ca3af", borderBottom: locTab === "region" ? "2px solid #508bf5" : "2px solid transparent", background: "none", border: "none", cursor: "pointer" }}>ì§€??„ ??/button>
                      <button onClick={() => setLocTab("keyword")} style={{ flex: 1, padding: "10px", fontSize: "14px", fontWeight: locTab === "keyword" ? 700 : 500, color: locTab === "keyword" ? "#508bf5" : "#9ca3af", borderBottom: locTab === "keyword" ? "2px solid #508bf5" : "2px solid transparent", background: "none", border: "none", cursor: "pointer" }}>?¤ì›Œ?œê???/button>
                    </div>
                    ...
                    {locTab === "region" ? (
                      <div>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "8px", maxHeight: "200px", overflowY: "auto" }}>
                          ...
                          )) : <div style={{ gridColumn: "1/-1", textAlign: "center", padding: "20px", color: "#9ca3af" }}>ë¡œë”©ì¤?..</div>)}
                          {regTab === "gugun" && (!selSidoCode ? <div style={{ gridColumn: "1/-1", textAlign: "center", padding: "20px", color: "#9ca3af" }}>???„ë? ë¨¼ì? ? íƒ?˜ì„¸??/div> : gugunList.length > 0 ? gugunList.map((c: any) => (
                          ...
                          {regTab === "dong" && (!selGugunCode ? <div style={{ gridColumn: "1/-1", textAlign: "center", padding: "20px", color: "#9ca3af" }}>??êµ?êµ¬ë? ë¨¼ì? ? íƒ?˜ì„¸??/div> : dongList.length > 0 ? dongList.map((c: any) => (
                      ...
                      <div>
                        <div style={{ display: "flex", gap: "8px", marginBottom: "12px" }}>
                          <input type="text" placeholder="?? ?? ë©??ëŠ” ?œë“œë§ˆí¬ ê²€?? value={locKeyword} onChange={e => setLocKeyword(e.target.value)} onKeyDown={e => e.key === "Enter" && doLocKeywordSearch()} style={{ flex: 1, padding: "10px 14px", border: "1px solid #d1d5db", borderRadius: "8px", fontSize: "14px", outline: "none" }} />
                          <button onClick={doLocKeywordSearch} style={{ padding: "10px 16px", background: "#508bf5", color: "#fff", border: "none", borderRadius: "8px", fontWeight: 700, fontSize: "14px", cursor: "pointer" }}>?´ë™</button>
                        </div>
          ...
          >
            ...
            {!mapLoaded && (
              ...
              >
                ...
                <p style={{ fontSize: "14px", color: "#9ca3af" }}>ì§€?„ë? ë¶ˆëŸ¬?¤ëŠ” ì¤?..</p>
              </div>
            )}
            ...
            {isLocating && (
              <div style={{ position: "fixed", inset: 0, background: "rgba(255,255,255,0.85)", backdropFilter: "blur(4px)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", zIndex: 9999 }}>
                ...
                <h3 style={{ fontSize: "18px", fontWeight: 800, color: "#111", marginBottom: "8px" }}>?„ì¬ ?„ì¹˜ë¥?ì°¾ê³  ?ˆìŠµ?ˆë‹¤</h3>
            ...
            {visibleArticles.length > 0 && mapLoaded && (
              <div style={{ position: "fixed", bottom: "calc(76px + env(safe-area-inset-bottom))", left: "50%", transform: "translateX(-50%)", zIndex: 20, display: "flex", justifyContent: "center" }}>
                ...
                >
                  ...
                  ì§€????ê¸°ì‚¬ {visibleArticles.length}ê°?
                </button>
              </div>
            )}
          </div>
          ...
          <div className={`news-detail-panel ${showListPanel ? "open" : ""}`} style={{ zIndex: 1500 }}>
            ...
            <div style={{ position: "sticky", top: 0, zIndex: 50, background: "#fff", display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", borderBottom: "1px solid #f0f0f0" }}>
              ...
              <h2 style={{ margin: 0, fontSize: 18, fontWeight: 800, color: "#111" }}>?´ë‹¹ ì§€??ê¸°ì‚¬ {listPanelArticles.length}ê°?/h2>
            </div>
              ...
              {listPanelArticles.map((article: any) => (
                ...
                >
                  <div style={{ display: "inline-flex", alignItems: "center", fontSize: "11px", fontWeight: 700, color: "#ea580c", border: "1px solid #fed7aa", padding: "2px 6px", borderRadius: "2px", marginBottom: "8px", alignSelf: "flex-start" }}>
                    {article.section1 || "ê³µì‹¤?´ìŠ¤"} {article.section2 && `> ${article.section2}`}
                  </div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", fontSize: "13px" }}>
                    <span style={{ color: "#222222", fontWeight: 500 }}>
                      {formatDate(article.published_at || article.created_at)} Â· {article.author_name || "ê³µì‹¤?´ìŠ¤"}
                      {article.location_name && ` Â· ?“${article.location_name}`}
                    </span>
                    <span style={{ color: "#f97316", fontWeight: 700 }}>ê¸°ì‚¬?ì„¸ë³´ê¸° &gt;</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div style={{ flex: 1, paddingBottom: "20px" }}>
          ...
          {(() => {
            ...
            const activeSub = section2Tab || "?„ì²´";
            const mentalText = PERSONALIZED_MENTAL_MAP[activeTab]?.[activeSub] || "ì¶”ì²œ ?´ìŠ¤";
            const displayName = memberName || "ë¶€?™ì‚°";
              ...
              <div style={{ padding: "20px 16px 12px", backgroundColor: "#fff", borderBottom: "8px solid #f4f6f8" }}>
                <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
                  <span style={{ fontSize: "13px", fontWeight: 600, color: "#6b7280", letterSpacing: "-0.3px" }}>
                    <span style={{ fontWeight: 800, color: "#111", background: "linear-gradient(180deg, transparent 50%, rgba(254, 240, 138, 0.9) 50%)", padding: "2px 4px", borderRadius: "2px" }}>{displayName} ?€?œë‹˜</span>???„í•œ
                  </span>
          ...
          {(initialKeyword || searchParams.get("keyword")) && (
            <div style={{ background: "#fff", display: "flex", flexDirection: "column", borderBottom: "1px solid #eee" }}>
              <div style={{ padding: "16px 16px 12px" }}>
                ...
                <span style={{ fontSize: "15px", fontWeight: 600, color: "#666", marginLeft: "6px" }}>ê²€?‰ê²°ê³?/span>
              </div>
              ...
              <div style={{ display: "flex" }}>
                <div 
                  ...
                  ê´€?¨ê¸°??<span style={{ color: searchTab === 'article' ? "#508bf5" : "#888" }}>{articles.length}</span>
                </div>
                <div 
                  ...
                  ê´€?¨ê³µ??<span style={{ color: searchTab === 'vacancy' ? "#508bf5" : "#888" }}>{vacancyCount}</span>
                </div>
              </div>
            </div>
          )}
            ...
            <div style={{ background: "#f9fafb", padding: "8px 16px 20px" }}>
                  ...
                  >
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "6px", flexWrap: "wrap" }}>
                        ...
                        {cardMasked && <span onClick={(e) => { e.stopPropagation(); setIsAuthModalOpen(true); }} style={{ fontSize: "11px", color: "#3b82f6", fontWeight: 700, background: "#eef6ff", padding: "3px 8px", borderRadius: "4px", cursor: "pointer" }}>?”’ ë¶€?™ì‚°?Œì› ë¬´ë£Œ?´ëŒ</span>}
                      </div>
                      ...
                      <p style={{ fontSize: "14px", color: "#6b7280", marginBottom: "2px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                        {[v.property_type || "ê±´ë¬¼", v.direction, v.exclusive_m2 && `${v.exclusive_m2}??].filter(Boolean).join(" | ")}
                      </p>
                      ...
                      <p style={{ fontSize: "14px", color: "#6b7280", marginBottom: "8px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                        {[v.room_count !== undefined ? `ë£?${v.room_count}ê°? : null, v.bath_count !== undefined ? `?•ì‹¤ ${v.bath_count}ê°? : null, ...(v.options || [])].filter(Boolean).join(", ")}
                      </p>
              ...
              {vacancyList.length === 0 && (
                <div style={{ textAlign: "center", padding: "60px 0", color: "#9ca3af" }}>
                  ...
                  <p style={{ fontSize: "15px", fontWeight: 600 }}>?´ë‹¹ ?¤ì›Œ?œì˜ ê³µì‹¤???†ìŠµ?ˆë‹¤.</p>
                </div>
              )}
            </div>
          )}
          ...
          {searchTab === 'article' && (() => {
            ...
            const currentCatLabel = section2Tab || (CATEGORIES.find(c => c.key === activeTab)?.label || "ê³µì‹¤?´ìŠ¤");
            ...
            return (
              <div>
                ...
                {/* ë§ì´ ë³??´ìŠ¤ ?œìœ„ ë¦¬ìŠ¤??(2ì°?ì¹´í…Œê³ ë¦¬ê°€ ?„ì²´???Œë§Œ ?¸ì¶œ) */}
                {section2Tab === "" && popularArticles.length > 0 && (
                  <div style={{ padding: "20px 16px", borderBottom: "8px solid #f4f6f8", backgroundColor: "#fff" }}>
                    <div style={{ display: "flex", alignItems: "center", marginBottom: "16px", paddingBottom: "8px", borderBottom: "1px solid #f3f4f6" }}>
                      ...
                      <span style={{ fontSize: "16px", fontWeight: 800, color: "#1f2937", marginLeft: "5px" }}>ë§ì´ ë³??´ìŠ¤</span>
                    </div>
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 16px 10px", backgroundColor: "#fff", borderBottom: "1px solid #f3f4f6" }}>
                  <span style={{ fontSize: "13px", fontWeight: 700, color: "#18181b", letterSpacing: "-0.3px" }}>
                    {KEY_TO_SECTION1[activeTab] || "ê³µì‹¤?´ìŠ¤"} <span style={{ color: "#a1a1aa", fontWeight: 400, margin: "0 2px" }}>&gt;</span> {section2Tab || "?„ì²´"}
                  </span>
                  <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                    <button 
          ...
          {!loading && searchTab === 'article' && articles.length === 0 && (
                <div style={{ textAlign: "center", padding: "60px 0", color: "#9ca3af" }}>
                  <div style={{ fontSize: "40px", marginBottom: "12px" }}>?“°</div>
                  <p style={{ fontSize: "15px", fontWeight: 600 }}>?„ì§ ê¸°ì‚¬ê°€ ?†ìŠµ?ˆë‹¤.</p>
                </div>
              )}
        </div>
      )}
      {/* ê¸°ì‚¬ ?ì„¸ ë·?(ëª¨ë°”???¬ë¼?´ë”© ?¨ë„) - ?°ë¦¬?™ë„¤?´ìŠ¤ ?„ìš© */}
        ...
        {detailLoading ? (
          ...
          <div style={{ padding: "0 20px 40px", backgroundColor: "#fff" }}>
            ...
            <div style={{ fontSize: "13px", color: "#666", marginBottom: "10px", marginTop: "16px" }}>
              [{articleDetail.section1 || "?´ìŠ¤"} &gt; {articleDetail.section2 || "?„ì²´"}]
            </div>
            {/* ?œëª© */}
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid #f0f0f0", paddingBottom: "16px", marginBottom: "20px" }}>
              <div style={{ fontSize: "13px", color: "#666" }}>
                <span style={{ fontWeight: 700, color: "#111", marginRight: "8px" }}>{articleDetail.author_name || "ê³µì‹¤?´ìŠ¤"}</span>
      ...
      <style>{`
        ...
        .news-detail-panel {
          ...
          margin-left: -224px; /* max-width 448px ???ˆë°˜ */
