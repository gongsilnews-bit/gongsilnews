...
const PERSONALIZED_MENTAL_MAP: Record<string, Record<string, string>> = {
  ...
  "news_etc": {
    "전체": "지역 부동산이 전하는 생생한 라이프 스토리",
    "인물/인터뷰": "다양한 삶의 현장 & 사람들의 생생한 인터뷰",
    "중개실무/인테리어Tip": "공실을 줄여주는 인테리어 & 실무 노하우",
    "맛집/여행/건강": "지역 부동산이 추천하는 동네 맛집 & 건강 팁",
    "스포츠/연예/기타": "일상에 즐거움을 더하는 소소한 이야기"
  }
};

const SUB_CATEGORY_PICTOGRAMS: Record<string, { label: string; icon: React.ReactNode }> = {
  // 공실현장
  "아파트/오피스텔": {
    label: "아파트·오피",
    icon: (
  ...
  "AI/NEWS": {
    ...
    icon: (
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        <line x1="9" y1="1" x2="9.01" y2="2"/>
        <line x1="15" y1="1" x2="15.01" y2="2"/>
        <line x1="9" y1="22" x2="9.01" y2="23"/>
        <line x1="15" y1="22" x2="15.01" y2="23"/>
        <line x1="1" y1="9" x2="2" y2="9.01"/>
        <line x1="1" y1="15" x2="2" y2="15.01"/>
        <line x1="22" y1="9" x2="23" y2="9.01"/>
        <line x1="22" y1="15" x2="23" y2="15.01"/>
      </svg>
    )
  },
  "부동산유튜브/블로그": {
...
const RecommendedNewsCarousel = React.memo(({ 
  ...
  // 🚀 [대표님 지시] 추천/중요 기사 클릭 시 네이버 뉴스 스타일 줌인 익스팬딩(Zoom-In Expanding) 트랜지션
  const [zoomingArticle, setZoomingArticle] = useState<{
    art: any;
    rect: { top: number; left: number; width: number; height: number };
    isExpanded: boolean;
  } | null>(null);

  // 추천 기사 백그라운드 프리페치 (즉각 전환 보장)
  React.useEffect(() => {
    if (!importantArticles || importantArticles.length === 0) return;
    importantArticles.slice(0, 8).forEach((art) => {
      router.prefetch(`/m/news/${art.article_no || art.id}`);
    });
  }, [importantArticles, router]);

  // 뒤로가기 시 줌인 오버레이 잔상 제거
  React.useEffect(() => {
    const handlePopState = () => setZoomingArticle(null);
    window.addEventListener("popstate", handlePopState);
    return () => window.removeEventListener("popstate", handlePopState);
  }, []);

  const handleImportantArticleClick = (e: React.MouseEvent<HTMLAnchorElement>, art: any) => {
    if (isSwipingRec) return;
    e.preventDefault();
    sessionStorage.setItem(`news_scroll_${activeTab}`, window.scrollY.toString());

    const rect = e.currentTarget.getBoundingClientRect();
    const targetUrl = `/m/news/${art.article_no || art.id}`;
    ...
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        setZoomingArticle((prev) => (prev ? { ...prev, isExpanded: true } : null));
      });
    });
      ...
      >
        {importantArticles.map((art, idx) => (
          <Link
            ...
            onClick={(e) => handleImportantArticleClick(e, art)}
            style={{
              textDecoration: "none",
              display: "flex",
      ...
      {zoomingArticle && (
        ...
        >
          ...
          <div
            style={{
              position: "absolute",
              top: zoomingArticle.isExpanded ? 0 : zoomingArticle.rect.top,
                  ...
                  >
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "6px", flexWrap: "wrap" }}>
                        ...
                        {cardMasked && (
                          <span onClick={(e) => { e.stopPropagation(); setSelectedVacancyId(v.id); }} style={{ fontSize: "11px", color: "#3b82f6", fontWeight: 700, background: "#eef6ff", padding: "3px 8px", borderRadius: "4px", cursor: "pointer" }}>
                    ...
                    {v.images?.[0] && (
                      <div style={{ width: "90px", height: "72px", borderRadius: "10px", overflow: "hidden", flexShrink: 0, backgroundColor: "#e5e7eb", alignSelf: "center", position: "relative" }}>
                        <Image src={v.images[0]} alt="" fill sizes="90px" style={{ objectFit: "cover" }} />
                      </div>
                    )}
