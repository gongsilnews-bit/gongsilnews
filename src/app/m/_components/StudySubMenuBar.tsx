...
export default function StudySubMenuBar({ activeTab, onTabChange, showBack }: Props) {
  ...
  const tabs: { key: StudyTab; label: string }[] = [
    { key: "lecture", label: "?¹ê°•ì¤€ë¹„ì¤‘" },
    { key: "board", label: "?ë£Œ?? },
    { key: "community", label: "ì»¤ë??ˆí‹°" },
  ];
  ...
  return (
    ...
    >
      ...
      {/* ?¤ë¡œê°€ê¸?ë²„íŠ¼ (ê²Œì‹œ???˜ì´ì§€?ì„œë§??œì‹œ) */}
      {showBack && (
        <button
          onClick={() => router.back()}
          style={{
        ...
        return (
          <button
            ...
            style={{
              ...
              padding: "8px 16px",
              borderRadius: "20px",
              border: `1.5px solid ${isActive ? "#1a2e50" : "#d1d5db"}`,
              backgroundColor: isActive ? "#1a2e50" : "#ffffff",
              fontSize: "13.5px",
              fontWeight: 700,
