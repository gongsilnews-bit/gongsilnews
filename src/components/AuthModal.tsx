...
export default function AuthModal({ isOpen, onClose, initialTab = 'signup', onGoogleClick }: AuthModalProps) {
  ...
  const modalContent = (
    ...
    >
      <style>{`.oauth-loading-dots span { opacity: 0.25; animation: auth-dot 1.2s infinite; } .oauth-loading-dots span:nth-child(2) { animation-delay: 0.2s; } .oauth-loading-dots span:nth-child(3) { animation-delay: 0.4s; } @keyframes auth-dot { 0%, 60%, 100% { opacity: 0.25; } 30% { opacity: 1; } }`}</style>
            ...
            <div style={{ padding: '40px 32px 32px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              ...
              <div style={{ width: 80, height: 80, borderRadius: '50%', border: '1px solid #eee', background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 24, boxShadow: '0 2px 6px rgba(0,0,0,0.06)', overflow: 'hidden' }}>
                <svg width="64" height="64" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <path d="M19 15.34L34 24L19 32.66Z" fill="#fbbf24" stroke="#222222" strokeWidth="3" strokeLinejoin="round" />
                </svg>
              </div>
              ...
              <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                <button onClick={() => handleOAuthLogin('google')} disabled={!!oauthLoading}
                  style={{ width: '100%', background: '#fff', border: '2px solid #4285F4', borderRadius: 8, padding: '13px 0', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, fontWeight: 'bold', fontSize: 15, color: '#222', boxShadow: '0 2px 8px rgba(66,133,244,0.15)', cursor: oauthLoading ? 'not-allowed' : 'pointer', opacity: oauthLoading && oauthLoading !== 'google' ? 0.5 : 1, fontFamily: 'inherit', transition: 'all 0.15s', position: 'relative' }}
                ...
                >
                  {!oauthLoading && <span style={{ position: 'absolute', top: -9, right: 14, background: '#4285F4', color: '#fff', fontSize: 10, fontWeight: 800, padding: '2px 8px', borderRadius: 10, letterSpacing: 0.5 }}>추천</span>}
                  {oauthLoading === 'google' ? <><span>로그인 중</span><span className="oauth-loading-dots"><span>.</span><span>.</span><span>.</span></span></> : <svg width="20" height="20" viewBox="0 0 48 48">
                    <path fill="#EA4335" d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.72 17.74 9.5 24 9.5z"/>
                    <path fill="#4285F4" d="M46.98 24.55c0-1.57-.15-3.09-.38-4.55H24v9.02h12.94c-.58 2.96-2.26 5.48-4.78 7.18l7.73 6c4.51-4.18 7.09-10.36 7.09-17.65z"/>
                    <path fill="#FBBC05" d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.27-3.14.76-4.59l-7.98-6.19C.92 16.46 0 20.12 0 24c0 3.88.92 7.54 2.56 10.78l7.97-6.19z"/>
                    <path fill="#34A853" d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.15 1.45-4.92 2.3-8.16 2.3-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48z"/>
                  </svg>}
                  {oauthLoading !== 'google' && 'Google 계정으로 시작하기'}
                </button>

                {/* 카카오 */}
                <button onClick={() => handleOAuthLogin('kakao')} disabled={!!oauthLoading}
                  style={{ width: '100%', background: '#FEE500', border: 'none', borderRadius: 8, padding: '13px 0', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, fontWeight: 'bold', fontSize: 15, color: '#000', cursor: oauthLoading ? 'not-allowed' : 'pointer', opacity: oauthLoading && oauthLoading !== 'kakao' ? 0.5 : 1, fontFamily: 'inherit', transition: 'all 0.15s' }}
                  onMouseOver={e => (e.currentTarget.style.background = '#f5dc00')}
                  onMouseOut={e => (e.currentTarget.style.background = '#FEE500')}
                >
                  {oauthLoading === 'kakao' ? <><span>로그인 중</span><span className="oauth-loading-dots"><span>.</span><span>.</span><span>.</span></span></> : <><svg width="20" height="20" viewBox="0 0 24 24"><path fill="#3C1E1E" d="M12 3C6.48 3 2 6.36 2 10.44c0 2.62 1.75 4.93 4.38 6.24l-1.12 4.16c-.1.36.3.65.6.44l4.94-3.26c.39.04.79.06 1.2.06 5.52 0 10-3.36 10-7.64C22 6.36 17.52 3 12 3z"/></svg>
                  카카오 계정으로 시작하기</>}
                </button>
              </div>
