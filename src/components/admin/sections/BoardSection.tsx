                  ...
                  <tr key={row.id || idx} style={{ borderBottom: `1px solid ${darkMode ? "#333" : "#f3f4f6"}`, transition: "background 0.15s" }}
                    ...
                    <td style={{ padding: "16px 20px", textAlign: "center", verticalAlign: "middle" }}>
                      <div style={{ display: "flex", gap: 8, justifyContent: "center" }}>
                        ...
                        <button onClick={async () => { if (confirm(`'${row.name}' 게시판을 삭제하시겠습니까?`)) { const res = await deleteBoard(row.board_id); if (res.success) setDbBoards(prev => prev.filter(b => b.board_id !== row.board_id)); else alert('삭제 실패: ' + res.error); }}} style={{ height: 30, padding: "0 12px", background: darkMode ? "#2c2d31" : "#fff", color: "#ef4444", border: `1px solid ${darkMode ? "#444" : "#d1d5db"}`, borderRadius: 4, fontSize: 12, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: 4, whiteSpace: "nowrap", flexShrink: 0 }}>
