            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                <tr key={a.id} style={{ borderBottom: `1px solid ${darkMode ? "#333" : "#f3f4f6"}` }}>
                  ...
                  <td style={{ padding: "16px 10px", textAlign: "center", verticalAlign: "middle", color: textSecondary, fontSize: 12 }}>{a.published_at ? (() => { const d = new Date(a.published_at); return `${d.toLocaleDateString('ko-KR', { year: 'numeric', month: '2-digit', day: '2-digit', timeZone: 'Asia/Seoul' })}\n${d.toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit', timeZone: 'Asia/Seoul', hour12: false })}`; })().split('\n').map((line, i) => <div key={i}>{line}</div>) : '-'}</td>
                  <td style={{ padding: "16px 10px", textAlign: "center", verticalAlign: "middle", color: textSecondary, fontSize: 12 }}>{a.updated_at ? (() => { const d = new Date(a.updated_at); return `${d.toLocaleDateString('ko-KR', { year: 'numeric', month: '2-digit', day: '2-digit', timeZone: 'Asia/Seoul' })}\n${d.toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit', timeZone: 'Asia/Seoul', hour12: false })}`; })().split('\n').map((line, i) => <div key={i}>{line}</div>) : '-'}</td>
                  <td style={{ padding: "16px 10px", textAlign: "center", verticalAlign: "middle" }}>
