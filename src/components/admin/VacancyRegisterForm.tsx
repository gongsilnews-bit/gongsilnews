...
export default function VacancyRegisterForm({ onBack, darkMode = false, userRole = "admin", initialClientName = "", initialClientPhone = "", ownerId = "", editData }: VacancyRegisterFormProps) {
  ...
  useEffect(() => {
    ...
    if (editData.metadata?.premium_fee) setPremiumFee(String(editData.metadata.premium_fee));
          ...
          <div style={{ background: cardBg, borderRadius: 14, padding: "36px 40px", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
            ...
            {(tradeType === "매매" && ["단독/다가구", "전원주택", "상가주택", "빌딩/건물", "공장/창고", "토지", "상가건물", "상가/업무"].includes(subCategory || "") || (propertyType === "상가·사무실·건물·공장·토지" && ["건물/빌딩", "공장/창고", "토지"].includes(subCategory || ""))) ? (
              ...
              <div style={{ display: "flex", gap: 24, marginBottom: isCommercial ? 24 : 16 }}>
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                    <input type="text" placeholder={propertyType === "상가·사무실·건물·공장·토지" ? "예: 1, B1, 1~2, 1층전체" : (propertyType === "빌라·주택" && ["단독/다가구", "전원주택", "상가주택"].includes(subCategory) ? "예: 일부, 1, 2, 전체" : "예: 3, 저층, 고층")} value={currentFloor} onChange={(e) => setCurrentFloor(e.target.value)} style={inputStyle} />
            ...
            <div style={{ display: "flex", gap: 24, marginBottom: 16 }}>
              ...
              <div style={{ flex: 1.5, display: "flex", flexDirection: "column" }}>
                ...
                <div
                  style={{
                    border: `2px dashed ${border}`, borderRadius: 12, padding: "24px 20px",
                    textAlign: "center", cursor: "pointer", flex: 1, minHeight: 120,
              ...
              const handleSubmit = async (customStatus?: string) => {
                ...
                try {
                  ...
                  const vacancyPayload: any = {
                    ...
                    commission_etc: commissionEtc || null,
                    supply_m2: supplyM2 ? parseFloat(supplyM2) : null,
                    supply_py: supplyPy ? parseFloat(supplyPy) : null,
                    exclusive_m2: exclusiveM2 ? parseFloat(exclusiveM2) : null,
                    exclusive_py: exclusivePy ? parseFloat(exclusivePy) : null,
                    room_count: isCommercial ? null : parseInt(roomCount) || 1,
                    bath_count: isCommercial ? null : parseInt(bathCount) || 1,
                    direction: isCommercial ? null : direction,
                    current_floor: currentFloor || null,
                    ...
                    sido, sigungu, dong,
                    detail_addr: detailAddr || null,
                    building_name: buildingName || null,
                    apt_dong: aptDong || null,
                    hosu: hosu || null,
                    ...
                    metadata: {
                      ...(editData?.metadata || {}),
                      land_share_m2: landShareM2 ? parseFloat(landShareM2) : null,
                      land_share_py: landSharePy ? parseFloat(landSharePy) : null,
                    ...
                    realtor_commission: userRole === 'realtor' ? (realtorCommission || null) : undefined,
                    exposure_type: userRole === 'realtor' ? (exposureType || null) : undefined,
                    landlord_name: userRole === 'realtor' ? (landlordName || null) : undefined,
                    landlord_phone: userRole === 'realtor' ? (landlordPhone || null) : undefined,
                    landlord_memo: userRole === 'realtor' ? (landlordMemo || null) : undefined,
                    consent,
                    infrastructure: Object.keys(infrastructure).length > 0 ? infrastructure : null,
                  };
