...
function VacancyDetailView({ vacancy, photos, isMyProperty, userLevel = 0, onBack, onInquiry }: { vacancy: any; photos: any[]; isMyProperty?: boolean; userLevel?: number; onBack: () => void; onInquiry?: (text: string) => void }) {
  ...
  const specs = [
    ...
    { label: vacancy.trade_type === "매매" && ((vacancy.property_type === "빌라·주택" && ["단독/다가구", "전원주택", "상가주택"].includes(vacancy.sub_category)) || (vacancy.property_type === "상가·사무실·건물·공장·토지" && ["건물/빌딩", "공장/창고", "지식산업센터"].includes(vacancy.sub_category))) ? "연면적" : "공급/전용", value: vacancy.trade_type === "매매" && ((vacancy.property_type === "빌라·주택" && ["단독/다가구", "전원주택", "상가주택"].includes(vacancy.sub_category)) || (vacancy.property_type === "상가·사무실·건물·공장·토지" && ["건물/빌딩", "공장/창고", "지식산업센터"].includes(vacancy.sub_category))) ? (vacancy.supply_m2 ? `${vacancy.supply_m2}㎡` : "-") : (vacancy.supply_m2 ? `${vacancy.supply_m2}㎡ / ${vacancy.exclusive_m2 || "-"}㎡` : "-") },
