...
function formatPrice(v: any): string {
  ...
  const fmt = (n: number) => {
    if (n >= 100000000) return `${(n / 100000000).toFixed(n % 100000000 === 0 ? 0 : 1)}??;
    if (n >= 10000) return `${Math.round(n / 10000)}留?;
    return `${n}`;
  };
  if (trade === "?붿꽭" && rent > 0) return `${fmt(dep)}/${fmt(rent)}`;
  if (trade === "?꾩꽭") return `?꾩꽭 ${fmt(dep)}`;
...
export default function MobileGongsilPage() {
  ...
  // 留덉빱 洹몃━湲?  useEffect(() => {
    ...
    // lat/lng 湲곗??쇰줈 洹몃９??    const groups: Record<string, any[]> = {};
    vacancies.forEach((v) => {
  ...
  const handleVacancyClick = async (v: any) => {
    ...
    setSelectedVacancy(v); // 癒쇱? 湲곕낯 ?뺣낫 ?쒖떆
    ...
    <div style={{ width: "100%", backgroundColor: "#fff", minHeight: "100vh", display: "flex", flexDirection: "column", position: "relative" }}>
      ...
      {!mapLoaded && (
        <div style={{ position: "absolute", inset: 0, background: "#e8ecf0", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 5 }}>
          <div style={{ textAlign: "center" }}>
            <div style={{ fontSize: "40px", marginBottom: "12px" }}>?뿺截?/div>
            <p style={{ color: "#6b7280", fontSize: "14px" }}>吏?꾨? 遺덈윭?ㅻ뒗 以?..</p>
          </div>
        </div>
      )}
      ...
      {mapLoaded && (
        <div style={{ position: "absolute", top: "16px", left: "16px", zIndex: 20, background: "rgba(255,255,255,0.95)", borderRadius: "20px", padding: "8px 14px", fontSize: "13px", fontWeight: 700, color: "#1a2e50", boxShadow: "0 2px 8px rgba(0,0,0,0.1)" }}>
          ?룫 怨듭떎 {vacancies.filter(v => v.lat && v.lng).length}嫄?        </div>
      )}
      ...
      <div className={`gongsil-sheet ${selectedCluster && !selectedVacancy ? "open" : ""}`} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ padding: "0 20px 14px", borderBottom: "1px solid #f3f4f6", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <h3 style={{ fontSize: "16px", fontWeight: 800, color: "#111827" }}>
            留ㅻЪ <span style={{ color: "#f97316" }}>{selectedCluster?.length || 0}</span>媛?          </h3>
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                ...
                <p style={{ fontSize: "12px", color: "#6b7280" }}>
                  {[v.exclusive_m2 && `?꾩슜 ${v.exclusive_m2}??, v.current_floor && `${v.current_floor}痢?, v.direction].filter(Boolean).join(" 쨌 ")}
                </p>
      ...
      <div className={`detail-panel ${selectedVacancy ? "open" : ""}`} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ position: "sticky", top: 0, zIndex: 10, background: "#fff", borderBottom: "1px solid #f3f4f6", display: "flex", alignItems: "center", gap: "12px", padding: "14px 16px" }}>
          ...
          <h2 style={{ fontSize: "16px", fontWeight: 800, color: "#111827", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            {selectedVacancy?.building_name || "留ㅻЪ ?곸꽭"}
          </h2>
        </div>
          ...
          <>
            ...
            <div style={{ padding: "20px 16px", borderBottom: "1px solid #f3f4f6" }}>
              <div style={{ display: "flex", gap: "6px", marginBottom: "12px" }}>
                ...
                {detailLoading && <span style={{ fontSize: "11px", color: "#9ca3af", alignSelf: "center" }}>?곸꽭 濡쒕뱶以?..</span>}
              </div>
              ...
              {selectedVacancy.maintenance_fee > 0 && (
                <p style={{ fontSize: "13px", color: "#6b7280", marginBottom: "8px" }}>愿由щ퉬 {Math.round(selectedVacancy.maintenance_fee / 10000)}留뚯썝</p>
              )}
            ...
            <div style={{ padding: "16px" }}>
              <h3 style={{ fontSize: "15px", fontWeight: 800, color: "#111827", marginBottom: "14px" }}>湲곕낯 ?뺣낫</h3>
              {[
                ["嫄곕옒諛⑹떇", selectedVacancy.trade_type],
                ["嫄대Ъ?좏삎", selectedVacancy.property_type],
                ["怨듦툒/?꾩슜", selectedVacancy.supply_m2 && `${selectedVacancy.supply_m2}??/ ${selectedVacancy.exclusive_m2 || "-"}??],
                ["?대떦/珥앹링", selectedVacancy.current_floor && `${selectedVacancy.current_floor}痢?/ ${selectedVacancy.total_floor || "-"}痢?],
                ["諛⑺뼢", selectedVacancy.direction],
                ["諛??뺤떎", selectedVacancy.room_count && `${selectedVacancy.room_count}媛?/ ${selectedVacancy.bath_count || 0}媛?],
                ["二쇱감", selectedVacancy.parking],
                ["?낆＜媛?μ씪", selectedVacancy.move_in_date || "利됱떆?낆＜(怨듭떎)"],
            ...
            {selectedVacancy.options?.length > 0 && (
              <div style={{ padding: "0 16px 16px" }}>
                <h3 style={{ fontSize: "15px", fontWeight: 800, color: "#111827", marginBottom: "12px" }}>?듭뀡</h3>
