...
(function initNaverBlog(global) {
  ...
  const DESIGNS = {
    basic: { label: "기본형", hint: "요약 인용구 · 물건표 · 큰 소제목. 어떤 물건에나 무난합니다." },
    magazine: { label: "매거진형", hint: "가운데 정렬 · 번호 소제목 · 핵심 문장 강조. 아파트·건물 물건에 어울립니다." },
  ...
  /* 링크: 물건 고정 상세 주소 → 없으면 처음 물건을 본 공실뉴스 주소 */
  ...
  /* 전송 전 검사: 경매·공매 물건 정보를 서버에서 받았는지 */
  function listingProblems(listing) {
    if (!listing) return ["경매·공매 물건 정보"];
    return listing.saleKind ? [] : ["경매·공매 구분"];
  }

  /* 글 끝 "물건 정보 출처" — 모든 디자인 공통.
     정보제공처(온비드 고객센터) 연락처와 입찰 전 법적 주의사항 안내문은 넣지 않는다. */
  function sourceBlockLines(listing, url) {
    const saleKind = listing?.saleKind || "경매";
  ...
  function buildNaverBlocks(body, media, options = {}) {
    ...
    if (design === "magazine") {
      ...
      if (rows.length) {
        html(paragraphHtml("[물건 정보]", { size: 17, bold: true }), { tight: true });
        html(tableHtml(rows));
      }
    }

    // 모든 디자인 공통: 물건 정보 출처 + 고정 상세 링크 + 공실뉴스 서명
    const source = sourceBlockLines(options.listing, url);
    html("<hr>");
    html(paragraphHtml("[물건 정보 출처]", { size: 13, color: MUTED, bold: true }), { tight: true });
    source.lines.forEach((line) => html(paragraphHtml(line, { size: 13, color: MUTED }), { tight: true }));
    html(BLANK, { tight: true });
    html(linkHtml(source.url, "▶ 공실뉴스 경매·공매 물건 상세 보기"));
