...
(() => {
  ...
  el.btnGoGongsil.addEventListener("click", () =>
    guard(el.btnGoGongsil, "공실열람 여는 중", async () => {
      ...
      if (found) {
        ...
        toast("공실열람으로 이동했습니다. [경매/공매] 메뉴에서 물건을 하나 펼쳐 주세요.", "info");
      }
  ...
  el.btnGrab.addEventListener("click", () =>
    guard(el.btnGrab, "물건 읽는 중", async () => {
      ...
      /* 경매 상세는 4개 탭으로 나뉘어 화면으로는 열린 탭만 읽힌다. 물건 정보는 서버에서 통째로 받는다. */
      S.vacancy = await fetchAuction(res.vacancy);
      renderVacancy(S.vacancy);

      const n = (S.vacancy.fields || []).length;
      toast(`${S.vacancy.saleKind || "경매"} 물건 정보 ${n}개 항목을 가져왔습니다. 이제 사진을 찍습니다.`, "ok");
      ...
      toast(
        `사진 ${S.media.length}장 준비됨 (물건 ${shots.filter((k) => k === "photo").length}장 · 검증 · 지도 · 로드뷰)`,
        "ok",
        5000
      );
      status("물건 준비됨", "ok");
  ...
  async function fetchAuction(domVacancy) {
    const id = domVacancy.vacancyId;
    if (!id) throw new Error("물건 번호를 읽지 못했습니다. 공실열람에서 물건을 다시 펼친 뒤 눌러 주세요.");
    ...
    if (!data?.success) {
      throw new Error((data && data.error) || "경매·공매 물건 정보를 가져오지 못했습니다. 잠시 뒤 다시 눌러 주세요.");
    }
  ...
  */
  ...
  $("btnReset").addEventListener("click", async () => {
    if (!confirm("가져온 물건, 초안, 사진, 블로그 글이 모두 지워집니다.\n초기화할까요?")) return;
    try {
  ...
  (async () => {
    await restore();
    refreshButtons();
    status(S.article ? "초안 있음" : S.vacancy ? "물건 준비됨" : "준비됨", S.article ? "ok" : "");
  })();
})();
