...
/* ══════════════════════════════════════════════════════════════
   블로그 초안 — 기사 원문을 복제하지 않고 독자에게 설명하는 글로 바꾼다
   ══════════════════════════════════════════════════════════════ */
function gwBuildBlogPrompt(vacancy, article, perspective) {
  const facts = vacancy ? gwFactLines(vacancy) : "- 별도로 가져온 매물 정보 없음";
  const view = String(perspective || "").trim();

  return `아래 공실뉴스 기사 초안을 바탕으로 네이버 블로그에 사람이 직접 검토해 올릴 블로그 초안을 작성하십시오.
...
[완성된 뉴스 기사]
제목: ${article.title || ""}
부제: ${(article.subtitles || []).join(" / ")}
본문:
${article.body || ""}

[작성자가 직접 입력한 경험·관점]
${view || "입력 없음 — 방문했다거나 직접 확인했다는 경험을 만들어 쓰지 마십시오."}

[가장 중요한 원칙]
- 뉴스 기사를 복사하거나 문장 끝만 바꾸지 말고, 독자가 매물을 검토할 때 도움이 되는 블로그 글로 새로 구성하십시오.
- 위 확인된 정보와 뉴스 기사에 없는 면적·금액·층수·주소·시설·시세를 만들어 내지 마십시오.
- 작성자 경험이 입력된 경우 의미를 바꾸지 말고 자연스럽게 반영하십시오.
- 경험 입력이 없으면 현장 방문, 직접 확인, 상담 경험을 한 것처럼 꾸미지 마십시오.
- 장점만 홍보하지 말고 어떤 사람에게 맞는지, 맞지 않을 수 있는지, 계약 전 확인할 점을 균형 있게 설명하십시오.

[검색 품질과 문체]
- 제목에서 [공실열람 보도] 같은 기사 말머리를 빼고 지역·매물종류·거래 특징을 자연스럽게 표현하십시오.
- 같은 키워드를 부자연스럽게 반복하거나 무관한 인기 검색어를 넣지 마십시오.
- 무조건, 확실한, 보장, 대박, 놓치면 후회 같은 과장·낚시 표현을 쓰지 마십시오.
- 친근하지만 가볍지 않은 부동산 전문가의 설명체로 쓰십시오.
- 1,200~1,800자, 짧은 문단 7~10개로 구성하십시오.
- 핵심 구간은 "■ 소제목"을 별도 문단으로 두어 모바일에서 읽기 쉽게 하십시오.
- 마지막에는 가격·거래조건·입주시기 등은 달라질 수 있으므로 게시 전 최신 정보를 다시 확인해야 한다는 안내를 넣으십시오.

[출력]
- keywords는 본문과 직접 관련된 지역명·매물종류·거래구분 중심 5~8개이며 #은 붙이지 마십시오.
- 설명 없이 아래 JSON 하나만 코드블록으로 출력하십시오.
- body는 문단별 문자열 배열이고 유효한 JSON이어야 합니다.
