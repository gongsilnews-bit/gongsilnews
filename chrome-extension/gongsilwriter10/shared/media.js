...
function gwProofCard(v) {
  return gwCard(
    [
      ...
      { text: gwField(v, "소재지") || "", size: 17, color: "#475569", weight: "600" },
      { text: gwField(v, "공실광고번호", "경매물건번호")
          ? "공실광고번호 " + gwField(v, "공실광고번호", "경매물건번호")
          : "", size: 15, color: "#64748b", weight: "600" },
    ],
    { badge: "공실뉴스 실매물 등록 검증", accent: "#2563eb" }
  );
}
...
function gwCaptionFor(kind, v, index = 0) {
  ...
  switch (kind) {
    case "proof":
      return `[공실뉴스 실매물 등록 검증] 공실뉴스에 공식 등록된 '${name}'${price ? ` (${price})` : ""} 등록 현황`;
    case "photo":
      return index === 0
