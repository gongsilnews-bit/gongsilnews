...
const GW_BLOG_STYLE = {
  ...
  visit: {
    label: "현장답사형",
    fit: "아파트·상가·빌딩",
    titleExample: "논현동 아크로힐스, 단지 안팎을 사진으로 살펴보니",
    guide:
  ...
  consult: {
    label: "친근한 해설형",
    fit: "실수요 매물",
    titleExample: "방 3개 아파트 매물, 어떤 가구에 맞을까",
    guide:
      "매물 조건을 독자가 이해하기 쉽게 풀어 설명하는 해설 기사로 쓴다. 부드러운 존댓말은 허용하지만 " +
      "권유하지 말고, 어떤 가구에 맞는 조건인지와 확인할 점을 객관적으로 짚는다.",
    design: "qna",
    structure:
  ...
  shortform: {
    label: "숏폼·속보형",
    fit: "SNS 연계 매물",
    titleExample: "[매물] 강남 논현동 아파트 15억 매매 등록",
    guide:
      "속보 기사처럼 첫 두 문장에 무엇이 어디에 얼마로 나왔는지를 짧게 전한다. 문장을 짧게 끊고 사실만 빠르게 전달하되, " +
...
- 이 글은 "공실뉴스에 이런 매물이 나왔다"는 객관적인 매물 보도입니다. 위 스타일은 전개 방식만 정하고 이 틀은 바꾸지 않습니다
- 3인칭 전달체로 쓸 것 (예: "~로 등록됐다", "~인 것으로 나타났다", "~로 확인된다"). 기본·분석·속보형은 기사체(~다), 현장답사·스토리텔링·해설형은 부드러운 존댓말(~습니다)도 허용
- 첫 문단에서 공실뉴스에 등록된 매물이라는 점과 소재지·매물 종류·거래 조건을 밝힐 것 (예: "공실뉴스에 서울 강남구 논현동 아파트 매매 매물이 등록됐다.")
- 출처를 분명히 할 것 (예: "공실뉴스에 등록된 매물 정보에 따르면")
- 권유·호객·광고 표현 금지: "보세요", "추천합니다", "문의하세요", "연락 주세요", "놓치지 마세요", "강력 추천", "지금이 기회" 등
- 독자에게 직접 권하는 2인칭 문장을 쓰지 말 것. 조언이 필요하면 "~라면 ~를 확인할 필요가 있다"처럼 객관적으로 쓸 것
- 위치·학군·지하철역·버스정류장·쇼핑시설·병원 등 주변 정보는 기사 가치가 있으므로 적극 전할 것. 단 [확인된 매물 정보]와 [참조 기사]에 있는 것만 쓰고, 자료에 없는 거리·도보 시간은 만들지 말 것
- 중개사무소 이름·연락처·링크는 본문에 쓰지 말 것 (글 끝 "매물 정보 출처"에 자동으로 들어갑니다)
...
- 마지막 문단은 매물 조건과 확인할 점을 객관적으로 정리하는 한 문단으로 쓸 것. 권유·연락처·문의 안내·링크는 쓰지 말 것
...
- "공실뉴스에 이런 매물이 나왔다"는 객관적 보도 형식을 유지하고, 권유·호객 표현이나 중개사무소 연락처를 넣지 마십시오.
