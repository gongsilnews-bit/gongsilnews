...
const SAMPLE_VACANCY = {
  priceText: "최저입찰가 291만원 (감정가 2억 9,100만원)",
  url: "https://gongsilnews.com/gongsil?id=1",
  fields: [
    { label: "감정가", value: "291,000,000원" },
    { label: "기타사항", value: "현장 확인 필요" },
    { label: "최저입찰가", value: "2,910,000원" },
  ],
};
...
test("기본형: 사진이 끼어들 때만 글을 끊고, 요약 인용구·표·구분선·소제목을 넣는다", () => {
  ...
  assert.ok(rest.startsWith("<table><tbody><tr><td><b>감정가</b></td><td>291,000,000원</td></tr><tr><td><b>최저입찰가</b></td>"), "출처 정보가 없으면 물건표 순서대로");
  assert.ok(!rest.includes("기타사항"), "표에는 정해 둔 핵심 항목만 넣는다");
...
const SAMPLE_LISTING = {
  ...
  saleKind: "공매",
  caseLabel: "관리번호",
  caseNo: "2023-08512-001",
  agency: "한국자산관리공사",
  capturedAt: "2026-09-25T03:00:00Z",
};

test("물건표는 소재지·물건종류 다음 가격·유찰·면적·일정·번호·기관 순서로, 개수 제한 없이", () => {
  const { factRows } = require("../shared/naver-blog.js");
  const fields = ["공고번호", "관리번호", "입찰 마감", "건물면적", "유찰 횟수", "감정가 대비 최저입찰가", "최저입찰가", "감정가", "구분", "기타사항", "집행기관", "입찰보증금"]
    .map((label) => ({ label, value: `${label}값` }));
  const rows = factRows({ fields }, SAMPLE_LISTING);
  assert.deepEqual(rows.map(([label]) => label), [
    "소재지", "물건종류", "구분", "감정가", "최저입찰가", "감정가 대비 최저입찰가", "유찰 횟수",
    "건물면적", "입찰 마감", "입찰보증금", "관리번호", "집행기관",
  ]);
});

test("모든 디자인 끝에 물건 정보 출처와 고정 상세 링크를 넣고, 온비드 연락처·법적 주의사항은 넣지 않는다", () => {
  ...
  for (const design of Object.keys(DESIGNS)) {
    ...
    assert.ok(html.includes("[물건 정보 출처]"), design);
    assert.ok(html.includes("공실뉴스 공실열람 공매 물건 정보 (2026.09.25 기준)"), design);
    assert.ok(html.includes("관리번호 2023-08512-001 | 집행기관 한국자산관리공사"), design);
    assert.ok(html.includes('<a href="https://gongsilnews.com/gongsil/detail/aa1c">'), design);
    assert.ok(!/1588-5321|고객센터|법적 책임/.test(html), `${design}: 온비드 연락처·법적 주의사항 없음`);
    assert.ok(!/문의는|문의하세요/.test(html), `${design}: 문의 유도 문구 없음`);
  }
});

test("경매·공매 물건 정보가 없으면 무엇이 빠졌는지 알려 준다", () => {
  const { listingProblems } = require("../shared/naver-blog.js");
  assert.deepEqual(listingProblems(null), ["경매·공매 물건 정보"]);
  assert.deepEqual(listingProblems(SAMPLE_LISTING), []);
  assert.deepEqual(listingProblems({ location: "x" }), ["경매·공매 구분"]);
});
