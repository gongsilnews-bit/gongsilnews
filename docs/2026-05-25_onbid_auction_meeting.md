...
## 📅 2026-05-27 추가 긴급 회의: 온비드 중복 매물 재발 원인 분석 및 완전 박멸 대책

### ① 🚨 중복 노출 현상 파악
* **상황**: 강남구 도곡동 `2025-15753-001` 매물 등 동일한 공고번호/물건번호를 가진 매물이 지도의 목록에 중복으로 2개씩 렌더링되고 있음.
* **원인 규명**: DB 쿼리(1:N 조인) 상의 문제가 아닌, **실제 데이터베이스(`vacancies` 테이블)에 동일한 매물이 2개 이상 중복 적재**되어 있는 것이 확인됨.

### ② 🔍 근본적인 기술적 원인 3가지
1. **행정구역(`sido`) 필터링 매칭의 텍스트 불일치 구멍 (핵심 원인)**
   * 동기화 엔진 `syncOnbidProperties`에서 중복 체크용 기존 데이터를 조회할 때 `eq("sido", targetSido.replace(...))` 처리를 사용함.
   * 온비드 API 원본(`targetSido`)이 `"인천광역시"`로 들어오면 위 처리 후 `"인천"`이 되지만, 실제 DB의 sido 컬럼에는 **`"인천"`** 또는 **`"경기"`** 등으로 저장되어 있음.
   * 하지만 `"경기도"`가 들어오는 경우 위 텍스트 가공을 거치면 `"경"`이 되는데, 실제 DB의 `sido` 값인 **`"경기"`**와 전혀 일치하지 않아(Mismatch) **DB에 엄연히 매물이 존재함에도 0건으로 판단**, "신규 매물"로 인식하고 동일 매물을 계속 **INSERT**하여 중복을 유발함.
2. **`metadata` 내 JSONB 값에 대한 Unique 제약조건의 부재**
   * 공고관리번호(`cltrMngNo`)가 컬럼이 아닌 `metadata` JSONB 내부에 들어가 있어 DB 레벨에서 고유 제약조건(Unique Key)으로 차단하지 못함.
3. **수동 및 배치 수집 실행 시 중복 제거 함수(`deduplicateOnbidProperties`) 자동 연동 누락**
   * 중복 매물을 깔끔하게 쓸어내는 강력한 정리 함수는 구현되어 있었으나, 동기화 스크립트 실행 직후에 유기적으로 연동되어 돌아가지 않아 누적된 중복이 청소되지 않음.

### ③ 🛠️ 철통 방어 및 박멸 해결책 반영 계획
1. **`sido` 텍스트 매칭 로직 정규화 보완**: `onbidSync.ts` 내부의 DB 조회 조건에서 `targetSido` 가공 방식을 행정구역 표준 매칭 함수(`normalizeSido` 규격)로 일원화하여 매칭 구멍 원천 봉쇄.
2. **동기화 엔진 실행 시 자동 중복 제거 연동**: `syncOnbidProperties` 동기화가 성공적으로 끝난 직후, 항상 중복 데이터를 스캔하여 1건만 남기고 지워버리는 `deduplicateOnbidProperties`를 **의무적으로 자동 트리거**하도록 동기화 파이프라인 개혁.
3. **즉각적인 데이터 대청소 실행**: 본 조치 즉시 전체 DB를 스캔하여 기존에 쌓인 중복 경매 매물을 단 0.1초 만에 깔끔히 대청소.
