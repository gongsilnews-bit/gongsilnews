...
### 2.2. 전문 매물 및 타겟 맞춤형 프리미엄 테마 라인업 스펙 (Theme Lineups)
중개사 및 제휴사의 주요 타겟 고객과 취급 매물의 성격에 딱 맞춘 **4대 특화 테마 라인업**을 기본 설계로 규격화합니다.

| 테마 명칭 (`theme_name`) | 주요 타겟 고객 (Target) | 디자인 특징 및 시각 요소 (Visual Identity) | 요금제 정책 (Gating) |
|:---|:---|:---|:---|
| **`buildon_vip`** <br>(빌디온 VIP 에디션) | 상가·빌딩·사무실 전문 중개소, 고급 오피스 컨설팅 | - **배경**: 다크 차콜 (Dark Mode 기본)<br>- **포인트 컬러**: 딥 골드 (#D4AF37)<br>- **레이아웃**: 극단적인 미니멀리즘 및 중후한 세리프 타이포그래피 | **최상위 프리미엄 전용** |
| **`cozy_nature`** <br>(내추럴 코지 에디션) | 아파트·재개발·단독주택·빌라 전문 패밀리 중개소 | - **배경**: 소프트 웜 화이트 (#FAF9F6)<br>- **포인트 컬러**: 올리브 그린 & 베이지<br>- **레이아웃**: 친근감을 주는 라운드 코너(Border-radius) 및 부드러운 그림자 | **기본 프리미엄 제공** |
| **`trendy_neon`** <br>(트렌디 네온 에디션) | 원룸·투룸·오피스텔 전문 중개소 (2030 청년층 타겟) | - **배경**: 퓨어 브라이트 화이트 (#FFFFFF)<br>- **포인트 컬러**: 비비드 퍼플 & 네온 블루<br>- **레이아웃**: 모바일 카드식 뷰 최적화, 스포티한 산세리프 및 볼드 헤드라인 | **기본 프리미엄 제공** |
| **`classic_trust`** <br>(클래식 트러스트 에디션) | 토지·공장·창고·분양 전문 묵직한 중개소 | - **배경**: 클린 그레이 & 화이트<br>- **포인트 컬러**: 딥 네이비 (#0F172A)<br>- **레이아웃**: 정보 탐색이 쉬운 격자 보드 레이아웃, 명확한 테이블 표 데이터 중심 | **기본 프리미엄 제공** |

#### 🎨 테마 스킨 분기 기술 메커니즘 (Theme Skinning Mechanism)
1. **CSS 전역 변수 동적 바인딩**:
   * 최상위 컨테이너 태그에 `theme_name` 값이 CSS 클래스로 자동 바인딩됩니다. (예: `<div className="theme-buildon-vip">`)
   * CSS(Vanilla/Tailwind)는 이를 기반으로 전역 디자인 토큰 변수를 실시간 변경합니다:
     ```css
     .theme-buildon-vip {
       --theme-bg: #121212;
       --theme-primary: #f39a11;
       --theme-text: #ffffff;
       --theme-radius: 4px;
     }
     .theme-cozy-nature {
     ...
     ```
2. **블록 컴포넌트 스킨 분기**:
   * 모든 블록 컴포넌트(예: 매물 카드 리스트)는 개별 테마용 코드를 따로 복제하여 관리하지 않고, 최상위 `--theme-*` CSS 변수를 100% 참조하여 렌더링되므로 성능 저하 및 중복 코드가 전혀 발생하지 않는 초경량 아키텍처를 실현합니다.
