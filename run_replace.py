...
def replace_in_file(filepath):
  ...
  const getPageStatus = (pageNum: number) => {
    ...
    # Normalize newlines for replacing
    old_text1 = old_text1.replace('\r\n', '\n')
    
    # Do replacing ignoring carriage returns
    content_normalized = content.replace('\r\n', '\n')
    if old_text1 in content_normalized:
        content_normalized = content_normalized.replace(old_text1, new_text1)
        print("Replaced 1")
    else:
        print("Failed to replace 1")

    # Replacement 2: PAGE 0 COVER
    old_text2 = '''    <div className="flex flex-col items-center p-8 bg-gray-100" ref={ref} style={themeStyles}>
        {/* PAGE 1: OVERVIEW */}
        {getPageStatus(1).shouldRender && ('''
    
    new_text2 = '''    <div className="flex flex-col items-center p-8 bg-gray-100" ref={ref} style={themeStyles}>
    ...
    old_text2 = old_text2.replace('\r\n', '\n')
    if old_text2 in content_normalized:
        content_normalized = content_normalized.replace(old_text2, new_text2)
        print("Replaced 2")
    else:
        print("Failed to replace 2")

    # Replacement 3: PAGE 7 CONTACT
    old_text3 = '''        </ReportPage>
        )}
    </div>
  );
});'''
    new_text3 = '''        </ReportPage>
        )}
    ...
    old_text3 = old_text3.replace('\r\n', '\n')
    if old_text3 in content_normalized:
        content_normalized = content_normalized.replace(old_text3, new_text3)
        print("Replaced 3")
    else:
...
replace_in_file('marketing/report/components/FlyerCanvas.tsx')
